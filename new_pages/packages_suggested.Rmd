...
* No código abaixo, os pacotes que são incluídos ao instalar/carregar outro pacote são indicados por um travessão e hash. Por exemplo, como **ggplot2** está listado em **tidyverse**.  
* Se vários pacotes têm funções com o mesmo nome, *masking* pode ocorrer quando a função do pacote mais recentemente carregado tem precedência. Leia mais na página [R- o básico]. Considere o uso do pacote **conflicted** para gerenciar tais conflitos.  
* Consulte a seção [R- o básico] sobre pacotes para mais informações sobre **pacman** e mascaramento.  
...
pacman::p_load(
     ...
     # aprendendo R
     ############
     learnr,   # tutoriais interetivos no RStudio 
