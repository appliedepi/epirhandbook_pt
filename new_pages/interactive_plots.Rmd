...
Cada vez mais tem sido necessária a visualização de dados, devido ao questionamento do público.Consequentemente, a criação de gráficos interativos torna-se  algo comum, do dia-a dia. Há várias formas de torná-los interativos, sendo as duas mais comuns: **plotly**  e **shiny**. 

Nesta página iremos focar na conversão de um gráfico já existente no `ggplot()`  em um gráfico interativo, utilizando o **plotly**.  Para saber mais sobre o **shiny**, veja na no capítulo [!Paineis com  Shiny](https://epirhandbook.com/en/dashboards-with-shiny.html#dashboards-with-shiny). Importante lembrar que este formato de gráfico interativo tem limitações, *utilizados apenas em documentos do R Markdown em formato HTML*, não sendo possível em documentos com formatos em PDF ou Word.
...
No exemplo abaixo, tem-se um gráfico de  curva epidêmica, que foi transformado para ser interativo,  usando a integração de **ggplot2** e **plotly**(_Passe o mouse sobre o grafico, amplie ou clique nos itens na legenda_). 
 ...
 ##*Atenção verificar se roda sem ter executados os códigos seguintes*
...
Nesta, parte de código(chunk),mostra o carregamento de pacotes necessários para as análises, com destaque `p_load()` do **pacman**, que instala o pacote se necessário e o carrega para uso. Ou você pode carregar utilizando a função `library()` - "Base do R". Para mais informações sobre pacotes consulte o capítulo  ![Noções básicas de R](https://epirhandbook.com/en/r-basics.html#r-basics). 
...
Assumindo que você está começando com um `ggplot()` gráfico que deseja converter para ser interativo. Vamos contruir então, vários gráficos, usando o caso`linelist`,  exemplo utilizado em outros capítulos anteriores deste manual.

### Importar os dados {.unnumbered}

Para começar, importe a lista de casos (limpa/organizada) de uma epidemia simulada de Ebola. Se quiser acompanhar o script, ![clique para baixar a linelist “limpa”] (https://github.com/appliedepi/epirhandbook_eng/raw/master/data/case_linelists/linelist_cleaned.rds),como arquivo .rds. Importe dados com a função  *import()* do pacote rio (ele lida com vários tipos de arquivos como .xlsx, .csv, .rds - veja a página, ![Importar e exportar](https://epirhandbook.com/en/import-and-export.html#import-and-export) para mais detalhes.
...
A função `ggplotly()` do pacote **plotly** facilita a conversão de um `ggplot()` para ser interativo.  Só precisa salvá-lo o seu `ggplot()` e na sequência direciona-lo para a função do `ggplotly()`.  
...
weekly_deaths <- linelist %>%
  ...
  summarise(                                              # criar um novo da frame (quadro)  de dados resumidos
    n_known_outcome = sum(!is.na(outcome), na.rm=T),      # número de casos por grupo(com resultados conhecidos/confirmados)
    n_death  = sum(outcome == "Death", na.rm=T),          # número de casos que foram a óbitos por grupo
    pct_death = 100*(n_death / n_known_outcome)           # percentual de casos que forma a obitos com resultados conhecidos/confirmados
  )
```
Aqui estão as primeiras 50 linhas do conjunto `weekly_deaths` do data.frame (dados).  

```{r message=FALSE, echo=F}
DT::datatable(head(weekly_deaths, 50), rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```
Na sequência o gráfico criado com  **ggplot2**, usando o `geom_line()`.  

```{r, warning=F, message=F}
deaths_plot <- ggplot(data = weekly_deaths)+            # construído com a semana do óbito
...
* Gire, ou mova-se pela plotagem clicando e arrastando a plotagem  
* Ampliar, reduzir ou retornar ao zoom padrão 
* Redefinir os eixos para os padrões 
* Ative/desative as “linhas pontiagudas” que são linhas pontilhadas do ponto interativo que se estendem até os eixos x e y
* Ajustes para mostrar os dados quando você não está pairando na linha  
...
Ao exportar em HTML gerado pelo R Markdown (como este livro!), você irá desejar o gráfico com o menor tamanho de dados possível (sem efeitos colaterais negativos na maioria dos casos). Para isso, basta direcionar a plotagem interativa para `partial_bundle()`,e para o **plotly**.  
...
Você pode tornar quase qualquer gráfico interativo do `ggplot()` , incluindo o gráfico de calor. No exemplo, a seguir mostra a proporção de dias por semana que certas instalações reportaram dados de malária  à sua província.

Aqui está o código, embora não descrito em profundidade neste tópico. 

```{r  message=F, warning=F}
# Importar os dados de malária
facility_count_data <- rio::import(here::here("data", "malaria_facility_count_data.rds")) #Ver capitulo 12 para baixar os dados de malária
...
<!-- Mapas também podem ser interativos  do  `ggplot()`  para o SIG, porém  mais trabalhoso-->
...
<!-- ## Aqui estão algumas das propriedades de cada elemento do objecto -->
...
<!-- ## Use {purrr} para aplicar esta função a cada elemento da lista de características do objecto geoJSON -->
...
<!-- plotly::plot_ly() %>%  -->
<!--   plotly::add_trace(         ##A função principal de mapeamento de parcelas -->
<!--     type="choropleth", -->
<!--     geojson=geoJSON, -->
