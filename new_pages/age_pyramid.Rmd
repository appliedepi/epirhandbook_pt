...
# Pirâmides demográficas e escalas Likert {}  
...
Para começar, importamos a lista limpa de casos de uma simulação de epidemia de Ebola. Se você quiser acompanhar, <a href='https://github.com/appliedepi/epirhandbook_eng/raw/master/data/case_linelists/linelist_cleaned.rds' class='download-button'> clique para baixar o linelist "clean" </a> (como um arquivo .rds). Importe dados com a função `import()` do pacote **rio** (ele lida com muitos tipos de arquivo como .xlsx, .csv, .rds - veja a página [Importar e exportar] para detalhes).  
...
* O argumento `data =` é definido como o quadro de dados `linelist`  
...
apyramid::age_pyramid(
  ...
  #pal = c("laranja", "roxo") # pode especificar cores alternativas aqui (mas não rótulos)
  ...                 
  
  # comandos ggplot adicionais
...
Os exemplos acima assumem que seus dados estão em um formato de linelist, com uma linha por observação. Se seus dados já estão agregados em contagens por categoria de idade, você ainda pode usar o pacote **apyramid**, conforme mostrado abaixo.  

Para demonstração, agregamos os dados da linelist em contagens por categoria de idade e sexo, em um formato "amplo" ou "wide". Isso simulará como se seus dados estivessem em contagens para começar. Saiba mais sobre [dados de agrupamento] e [dados dinâmicos] em suas respectivas páginas.  
...
Para configurar esses dados para a pirâmide de idade, vamos dinamizar os dados para serem "longos" com a função `pivot_longer()` de **dplyr**. Isso ocorre porque `ggplot()` geralmente prefere dados "longos", e **apyramid** está usando `ggplot()`.  
...
```{r, warning=F, message=F, echo=F}
  # começar ggplot
  ggplot(mapping = aes(x = age, preenchimento = gender)) +
...
# começar ggplot
  ...
  scale_y_continuous(
    ...
    break = seq(from = floor (min_per), # sequência de valores, por 2s
                 to = ceiling(max_per),
                 by = 2),
...
#######################################################
case_data <- linelist%>%
  cout(age_cat5, gender, name = "counts")%>% # contagens por grupos de idade-gênero
  ungroup() %>% 
  mutate(
...
Agora os dois quadros de dados estão combinados, um em cima do outro (eles têm os mesmos nomes de coluna). Podemos "nomear" cada frame de dados e usar o argumento `.id =` para criar uma nova coluna "data_source" que indicará de qual frame de dados cada linha se originou. Podemos usar esta coluna para filtrar no `ggplot()`.  
...
ggplot() + # eixo x padrão é a idade em anos;
  ...
  # manualmente garantir que o eixo da idade está ordenado corretamente
  scale_x_discrete(limits = age_levels) + # definido no bloco acima
  
  # definir eixo percentual 
  scale_y_continuous(
    limites = c(min_per, max_per), # min e max definidos acima
  ...
  # designar cores e rótulos de legenda manualmente
  scale_fill_manual(
    valores = c("f" = "laranja", # atribuir cores aos valores nos dados
               "m" = "darkgreen"),
    labels = c("f" = "Feminino",
...
ggplot()+
  ...
  # gráfico de barras das respostas "negativas" 
     geom_bar(
       ...
       mapping = aes(
         x = status,
         y = ..contagem.. * (- 1), # contagens invertidas para negativas
         fill = Response),
       color = "black",
       closed = "left",
       position = "stack")+
    ...
    scale_y_continuous(
      ...
      break = seq(from = -ceiling (melted_max / 10) * 10,
                   to = ceiling(melted_max / 10) * 10,
                   by = 10),
      
      # rótulos da escala do eixo x
      labels = abs(unique(c(seq(-ceiling(melted_max/10)*10, 0, 10),
                            seq(0, teto (derretido_max / 10) * 10, 10))))) +
