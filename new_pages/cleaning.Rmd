# Limpeza de dados e principais funções
...
Este manual emfatiza a utilização das funções da família [**tidyverse**](https://www.tidyverse.org/) de pacotes do R. As funções essenciais do R demonstradas nesta página estão listadas abaixo.

Muitas destas funções pertencem ao pacote do R [**dplyr**](https://dplyr.tidyverse.org/), que fornece funções "verbo" para resolver desafios de manipulação de dados (o nome é uma referência a um "quadro de dados -[plier](https://www.thefreedictionary.com/plier#:~:text=also%20ply%C2%B7er%20(pl%C4%AB%E2%80%B2,holding%2C%20bending%2C%20or%20cutting.)%22). **dplyr** é parte da família **tidyverse** de pacotes do R (os quais também incluem **ggplot2**, **tidyr**, **stringr**, **tibble**, **purrr**, **magrittr**, e **forcats** entre outros).

+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
| Função                                              | Utilidade                                                                 | Pacote                     |
+=====================================================+===========================================================================+============================+
| `%>%`                                               | "pipe" (passar) dados de uma função para a próxim                         | **magrittr**               |
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
...
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
| `as.character()`, `as.numeric()`, `as.Date()`, etc. | converter a classe de uma coluna                                          | **base** R                 |
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
...
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
| `distinct()`                                        | desduplicar as linhas                                                     | **dplyr**                  |
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
| `rowwise()`                                         | operações po/em cada linha                                                | **dplyr**                  |
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
...
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
| `age_categories()` and `cut()`                      | criar grupos categóricos de uma coluna numérica                           | **epikit** e **base** do R |
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
| `clean_variable_spelling()`                         | recodificar/limpar valores usando um dicionário de dados                  | **linelist**               |
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+
| `which()`                                           | aplicar critérios lógicos; retorna índices                                | **base** do R              |
+-----------------------------------------------------+---------------------------------------------------------------------------+----------------------------+

Se quiser ver como estas funções se comparam aos comandos Stata ou SAS, consulte a página em [Transição para o R].

Você poderá encontrar uma estrutura alternativa de gestão de dados a partir do pacote do R **data.table** com operadores como `:=` e utilização frequente de colchetes `[ ]`. Esta abordagem e sintaxe é brevemente explicada na página [Tabela de dados].

### Nomenclatura {.unnumbered}

Neste manual, referimos geralmente "colunas" e "linhas" em vez de "variáveis" e "observações". Como explicado neste manual sobre ["dados arrumados"](https://tidyr.tidyverse.org/articles/tidy-data.html), a maioria dos conjuntos de dados estatísticos epidemiológicos consistem estruturalmente em linhas, colunas e valores.

*Variáveis* ontêm os valores que medem o mesmo atributo subjacente (como o grupo etário, desfecho, ou data de início) *Observações* contêm todos os valores medidos na mesma unidade (por exemplo, uma pessoa, local, ou amostra de laboratório). Portanto, estes aspectos podem ser mais difíceis de definir de forma tangível.

Em conjunto de dados "arrumados",cada coluna é uma variável, cada linha é uma observação, e cada célula é um único valor. Contudo, alguns conjuntos de dados que você encontra não se enquadrarão neste molde - um conjunto de dados de formato "amplo" pode ter uma variável dividida em várias colunas (ver um exemplo na página [Dados Pivotantes]). Da mesma forma, as observações podem ser divididas em várias linhas.

A maior parte deste manual trata da gestão e transformação de dados, referindo assim às estruturas de dados concretos de linhas e colunas é mais relevante do que as observações e variáveis mais abstractas. As excepções ocorrem principalmente em páginas sobre análise de dados, onde se verá mais referências a variáveis e observações.
...
Este pedaço de código mostra o carregamento das pacotes necessários para as análises. Neste manual, damos ênfase a `p_load()` do **pacman**, que instala o pacote se necessário *e* carrega-o para uso. Você pode também carregar pacotes instalados com `library()` da **base** do R. Veja a página do [Básico do R] para mais informações do pacotes do R.
...
Se o seu conjunto de dados for grande e demorar muito tempo a importar, pode ser útil que o comando de importação seja separado da cadeia pipe e que o "bruto" seja guardado como um ficheiro distinto. Isto também permite uma comparação fácil entre a versão original e a versão limpa.
...
Você pode visualizar as primeiras 50 linhas do quadro de dados abaixo. Nota: a função do pacote **base** do R `head(n)` alpermite que você visualize apenas as primeiras `n` linhas no console do R.
...
-   Nomenclatura de estilo similar (por exemplo, todas as colunas de datas nomeadas como **data\_**início, **data\_**relato, **data\_**morte...)

Os nomes das colunas de `linelist_raw` são impressos abaixo utilizando `names()` da **base** do R. Podemos ver isso inicialmente:

-   Alguns nomes contêm espaços (por exemplo, `data de infecção`)\
-   Diferentes padrões de nomes são utilizados para datas (`data de início` vs. `data de infecção`)\
-   Deve ter havido um *cabeçalho combinado* nas duas últimas colunas no .xlsx. Sabemos disso porque o nome de duas colunas combinadas ("cabeçalho_combinado") foi atribuído pelo R à primeira coluna, e à segunda coluna foi atribuído um nome de espaço reservado "...28" (pois estava então vazia e é a 28ª coluna).

```{r}
names(linelist_raw)
```

[***NOTA:*** Para referenciar um nome de coluna que inclua espaços, rodeie o nome com apóstrofos, por exemplo: linelist\$`` ` '\x60infection date\x60'` ``. Note que no seu teclado, o apóstrofo (\`) é diferente das aspas simples (').]{style="color: black;"}
...
```{r}
# CADEIA DE 'PIPE' LIMPA (inicia com dados brutos e pipes através de etapas de limpeza)
##################################################################################
linelist <- linelist_raw %>%
...
[***CUIDADO:*** Nos exemplos abaixo, o quadro de dados `linelist` é modificado com `select() e exibido, mas não salvo. Isto é para fins de demonstração. Os nomes das colunas modificadas são impressos por piping o banco de dados para `names()`.]{style="color: orange;"}


**Aqui estão TODOS os nomes de colunas na lista de linhas, neste ponto da cadeia limpa de pipe:**
...
```{r}
# O conjunto de dados linelist é canalisado através do comando select(), e names() imprimem apenas os nomes das colunas
linelist %>% 
...
-   `where()` - aplica uma função a todas as colunas e seleciona aquelas que são VERDADEIRAS\
...
-   `any_of()` - corresponde SE a coluna existir, mas não retorna nenhum erro se não for encontrada
...
A função `matches()` funciona de forma semelhante a `contains()` mas pode ser fornecida uma expressão regular (ver página em [Caracteres e strings]), tais como múltiplos strings separados por OR barras dentro dos parênteses:
...
Você também pode remover uma coluna utilizando a síntaxe da **base** do R, definindo-a como `NULL`. Por exemplo:
...
```{r}
# CADEIA 'PIPE' LIMPA (inicia com dados brutos e pipes através das etapas de limpeza)
##################################################################################
...
Consulte a página do manual de [De-duplicação] para obter opções extensivas sobre como de-duplicar os dados. Apenas um exemplo muito simples de desduplicação é apresentado aqui.
...
Após a desduplicação, existem `nrow(linelist)` linhas. Qualquer linha removida teria sido 100% duplicada de outras linhas.

Abaixo, o comando `distinct()` é adicionado à cadeia de pipe de limpeza:

```{r}
# CADEIA 'PIPE' DE LIMPEZA (inicia com dados brutos e pipes através das etapas de limpeza)
##################################################################################
...
Abaixo está um exemplo de criação de uma nova coluna com `mutate()`. A sintaxe é: `mutate(new_coluna_name = value or transformation)`
...
Você também pode referenciar valores em outras colunas, para realizar cálculos. Abaixo, uma nova coluna `bmi` é criada para manter o Índice de Massa Corporal (IMC) para cada caso - como calculado utilizando a fórmula IMC = kg/m\^2, utilizando coluna `ht_cm` e coluna `wt_kg`.
...
Colunas contendo valores que são datas, números ou valores lógicos (VERDADEIRO/FALSO) só se comportarão como esperado se forem corretamente classificadas. Há uma diferença entre "2" da classe caracteres e 2 da classe numérica!

Há maneiras de definir a classe da coluna durante os comandos de importação, mas isso é muitas vezes complicado. Veja a seção [Básico do R] sobre classes de objetos para saber mais sobre a conversão da classe de objetos e colunas.
...
Em um modo similar, vocÊ pode usar `as.character()` e `as.logical()`. Para converter para a classe Fator, você pode usar `factor()` da **base** do R ou `as_factor()` do **forcats**. Leia mais sobre isso na página [Fatores].
...
Especifique as colunas para o argumento `.cols =`. Você pode nomeá-las individualmente, ou usar as funções do helper do "tidyselect". Especifique a função para .fns =`. NObserve que utilizando o modo de função demonstrado abaixo, a função é escrita *sem* seus parênteses ( ).
...
As funções do helper "tidyselect" estão disponíveis para ajudá-lo a especificar colunas. Elas são detalhadas acima na seção sobre Seleção e reordenação de colunas, e eles incluem: `everything()`, `last_col()`, `where()`, `starts_with()`, `ends_with()`, `contains()`, `matches()`, `num_range()` e `any_of()`.
...
```{r, eval=F}
#Mudar todas as colunas para classe de caracteres
linelist <- linelist %>% 
  mutate(across(.cols = contains("date"), .fns = as.character))
```

Abaixo, um exemplo de mutação de colunas que atualmente são da classe POSIXct (uma classe de data/hora bruta que mostra os carimbos temporais) - em outras palavras, onde a função `is.POSIXct()` avalia para `TRUE`. Em seguida, queremos aplicar a função `as.Date()` a estas colunas para convertê-las em uma classe normal Date.
...
- Note que `is.POSIXct()` é do pacote **lubridate**. Outras funções "is" similares como `is.character()`, `is.numeric()`, e `is.logical()` são da **base do R***.
...
-   Você pode fornecer a função no estilo **purrr** (por ex.,`~ mean(.x, na.rm = TRUE)`) (veja [esta página][Interações, loops, e listas])\
...
Esta função **dplyr** ncontra o primeiro valor não perdido em cada posição. Ela "preenche" os valores ausentes com o primeiro valor disponível em uma ordem especificada por você.

Aqui está um exemplo *fora do contexto de um data frame*: Digamos que você tem dois vetores, um contendo a vila de detecção do paciente e outro contendo a vila de residência do paciente. Você pode usar a coalesce para escolher o primeiro valor não desconsiderado para cada índice:
...
Isto funciona da mesma forma se você fornecer colunas de data frame: para cada linha, a função atribuirá o novo valor de coluna com o primeiro valor não descartado nas colunas que você forneceu (em ordem fornecida).
...
### Usando a **base** do R {.unnumbered}

Para definir uma nova coluna (ou redefinir uma coluna) utilizando a **base** do R, escreva o nome do data frame, conectado com `$`, para a *nova* coluna (ou a colunas a ser modificada). Use o operador de atribuição `<-` para definir o(s) novo(s) valor(es). Relembre que quando usar a **base** do R voê deve especificar o nome do data frame antes do nome da coluna toda vez (por ex., `dataframe$column`). Aqui é um exemplo de criação da coluna `bmi` usando a **base** do R:
...
```{r }
# CADEIA 'PIPE' LIMPA (começa com dados brutos e as pipes através das etapas de limpeza)
##################################################################################
...
Imagine que haja uma data sem sentido nos dados (por exemplo, "2014-14-15"): você poderia fixar a data manualmente nos dados da fonte bruta, ou, você poderia escrever a alteração na tubulação de limpeza via `mutate()` e `recode()`. Esta última é mais transparente e reprodutível para qualquer outra pessoa que procure entender ou repetir sua análise.
...
A linha `mutate()` aciima pode ser lida como: "mude a coluna `date_onset` para igualar a coluna `date_onset` re-codificada para que o VALOR VELHO seja alterado para o NOVO VALOR". Note que este padrão (VELHO = NOVO) para `recode()` é o oposto da maioria dos padrões de R (novo = velho). A comunidade de desenvolvedores do R está trabalhando na revisão deste padrão.
...
Para re-codificar com critérios lógicos simples, você pode utilizar `replace()` dentro de `mutate()`. `replace()` é uma função da **base*** do R. Utilize uma condição lógica para especificar as linhas a serem alteradas . A sintaxe geral é:

`mutate(col_to_change = replace(col_to_change, critério para linhas, new value))`.

Uma situação comum para utilizar `replace()` é **alterar apenas um valor em uma linha, utilizando um identificador único de linha**. Abaixo, o gênero é alterado para "Female" na linha onde a coluna `case_id` é "2195".

```{r, eval=F}
#  Exemplo: mudar o gênero de uma observação específica para "Feminino".  
linelist <- linelist %>% 
  mutate(gender = replace(gender, case_id == "2195", "Female"))
```

O comando equivalente utilizando a síntaxe da **base** do R e colchetes indexadores `[ ]` está abaixo. Ele é lido como "Alterar o valor da coluna `gender` do dataframe `linelist` (para as linhas onde a coluna `case_id` do `linelist` tem o valor de '2195') para 'Female' ".
...
Fora do contexto de um data frame, se você quiser que um objetou tilizado em seu código mude seu valor, considere utilizar  `switch()` da **base** do R.
...
Os comandos `case_when()` consistem de declarações que têm um Lado Direito (RHS) e um Lado Esquerdo (LHS) separados por um "til" `~`. Os critérios lógicos estão no lado esquerdo e os valores de ajuste estão no lado direito de cada afirmação. As afirmações são separadas por vírgulas.
...
linelist <- linelist %>% 
  mutate(age_years = case_when(
            ...
            TRUE                 ~ NA_real_)) # qualquer outra circunstância, atribuir falta
```
...
Nesta linha, em sua declaração final, coloque `TRUE` no lado esquerdo, que captará qualquer linha que não atenda a nenhum dos critérios anteriores. Ao lado direito desta declaração pode ser atribuído um valor como "check me!" ou perda.

[***PERIGO:*** **Valores no lado direito devem ser todos na mesma classe** - seja numérico, caractere, data, lógico, etc. Para atribuir valores em falta (`NA`), você talvez necessite usar as variações especiais de `NA` como `NA_character_`, `NA_real_` (para numérico ou POSIX), e `as.Date(NA)`. Leia mais em [Trabalhando com datas].]{style="color: red;"}

### Valores perdidos {.unnumbered}

Abaixo estão as funções especiais para manipular valores ausentes no contexto da limpeza de dados.

Veja a página em [Dados ausentes] para dicas mais detahadas sobre como identificas e tratar os valores ausentes. Por exemplo, a função `is.na()`que logicamente testa  a falta de dados.
...
Esta é uma função do pacote **forcats**. O pacote **forcats** manipula colunas de classe Fator. Fatores são a forma do R manipular valores *ordenados* tais como `c("First", "Second", "Third")` ou para definir a ordem em que os valores (por exemplo, hospitais) aparecem em tabelas e gráficos. Veja a página de [Fatores].
...
A maneira mais fácil de resolver isto é utilizar a função de **forcats** `fct_explicit_na()` que converte uma coluna para a classe fator, e converte os valores `NA` para o caractere "(Ausente)".
...
Utilize o pacote R **linelist** e sua função `clean_variable_spelling()` para limpar um data frame com um *dicionário de limpeza*. **linelist** é um pacote desenvolvido por [RECON](https://github.com/reconhub/linelist) - o Consórcio R Epidemics.
...
2)  Importar o arquivo do dicionário para R. Este exemplo pode ser baixado através das instruções na página [Baixar manual e dados].
...
3)  Passe a linelist bruta para `clean_variable_spelling()`, especificando para `wordlists =` o data frame do dicionário de limpeza. O argumento `spelling_vars =` pode ser usado para especificar qual coluna do dicionário se refere às colunas (3º por padrão), ou pode ser definido como `NULL` para que o dicionário se aplique a todas as colunas de caracteres e fatores. Note que esta função pode levar muito tempo para ser executada.

```{r}
linelist <- linelist %>% 
  linelist::clean_variable_spelling(
    wordlists = cleaning_dict,
    spelling_vars = "col",        # coluna do dicionário contendo nomes das colunas, padrão da 3ª coluna do dicionário
  )
```
...
Observe que os nomes das colunas no dicionário de limpeza devem corresponder aos nomes *neste ponto* do seu acripr de limpeza. Veja esta [referência online reference para o pacote linelist ](https://www.repidemicsconsortium.org/linelist/reference/clean_data.html) para obter mais detalhes.
...
```{r}
# LIMPEZA DA CADEIA 'PIPE' (inicie com dados brutos e canalize-os através de etapas de limpeza)
##################################################################################
...
Primeiro, examine a distribuição de seus dados, para fazer os pontos de corte apropriados. Veja a página sobre [ggplot basics].
...
[***CUIDADO:*** Às vezes, variáveis numéricas serão importadas como classe "character". sso ocorre se houver caracteres não numéricos em alguns dos valores, por exemplo, uma entrada de "2 meses" para idade ou (dependendo das configurações locais do R) se uma vírgula for usada na casa decimal (por exemplo, "4,5" para significar quatro anos e meio)..]{style="color: orange;"}
...
`cut()` é uma alternativa da **base** do R para `age_categories()`, mas eu acho que você verá porque `age_categories()` foi desenvolvida para simplificar este processo. Algumas diferenças notáveis da `age_categories()` são:
...
**Rotulem novamente os valores `NA`**
...
linelist <- linelist %>% 
  ...
  mutate(age_cat = cut(
    ...
    age_cat = fct_explicit_na(
      age_cat,
      na_level = "Missing age")  # você pode especificar o rótulo
  )    
...
Para uma maneira rápida de fazer quebras e rotular vetores, use algo como abaixo. Veja a página [Básico do R] para referências em `seq()` e `rep()`.
...
### Quebras quantílicas {.unnumbered}

No entendimento comum, “quantis” ou “percentis” normalmente se referem a um valor abaixo do qual uma proporção de valores cai. Por exemplo, o 95º percentil de idades linelistseria a idade abaixo da qual 95% da idade é inferior.
...
# combine e imprima - note que valores estão presentes em múltiplos grupos
...
Se utilizar `case_when()`, por favor, revise o uso apropriado conforme descrito anteriormente na seção Re-codificar valores desta página. Esteja ciente também que todos os valores do lado direito devem ser da mesma classe. Assim, se você quiser `NA` no lado direito, você deve escrever "Missing" ou utilizar o valor `NA` especial  `NA_character_`.
...
Antes do filtri ser aplicado, o número de linhas da `linelist` é `nrow(linelist)`.
...
#### Deesenhe o filtro {.unnumbered}

Examine a tabulação cruzada para ter certeza que excluiímos somente as linhas corretas:
...
Começamos com uma linelista de linha(linelist){\i1}`. Aqui está nossa declaração de filtro:

```{r}
linelist <- linelist %>% 
  # mantém linhas onde o início é depois de 1 de Junho OU o início dos sintomos está faltante E o hospital é OUTRO que não A nem B 
...
Muitas vezes você quer rever rapidamente alguns registros, para apenas algumas colunas. A função do R **base** `View()` imprimirá um quadro de dados para visualização em seu RStudio.
...
Dentro de `View()`, encadeie o conjunto de dados com o pipetáveis (%>%) com a função `filter()` para manter certas linhas, e depois com a função `select()` para manter certas colunas. Por exemplo, para rever as datas de início e hospitalização de 3 casos específicos:
