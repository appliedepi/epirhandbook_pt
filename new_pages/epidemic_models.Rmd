...
Você também pode carregar pacotes instalados com `library()`, do R **base**. Veja a página sobre [Básico do R] para mais informações sobre pacotes no R.
...
* **EpiNow2** é significamente mais devagar do que **EpiEstim**, por fatores entre
  ...
  número de iterações para garantir elevada acurácia, e provavelmente poderia ser resudiza caso
  necessário. Entretanto, o ponto é que este algorítmo é mais devagar,
...
seus dados. Essencialmente, você precisar ser capaz de descrever o intervalo ebtre a data
...
objeto epicontact (veja a página [Cadeias de transmissão] para mais
detalhes):
...
## crie um df longo para criar gráfico de quantils
df_long <- df_wide %>%
...
o intervalo seriado (ex.: a distribuição dos intervalos entre o início dos sintomas dos
...
(veja a página [Cadeias de transmissão] para mais detalhes):
...
df_wide <- epiestim_res_lit$R %>%
  ...
  ## atribua os níveis de factor
  mutate(variable = factor(variable, c("risk", "r")))

## crie um quadro de dados longo a partir dos quantis
df_long <- df_wide %>%
...
#### Projetando a incidência {.unnumbered}
