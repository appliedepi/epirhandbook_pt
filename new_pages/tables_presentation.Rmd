...
Instalar e carregar o pacote **flextable**. Neste manual, nós destacamos o `p_load()` do **pacman**, o qual instala o pacote, se necessário, *e* o carrega para ser utilizado. Você também pode carregar os pacotes com `library()` presente no R **base**. Veja a seção sobre [R - o básico] para obter mais informações sobre os pacotes “R”.
...
*Antes* de começar a utilizar o pacote **flextable** você precisará *criar* a sua tabela como um data frame. Veja a seção em [Tabelas descritivas] e [Pivotando dados] para aprender a criar um quadro de dados utilizando pacotes como **janitor*** e **dplyr***. 
...
Abaixo está um exemplo da página [Tabelas descritivas] de conversão dos casos na `linelist` dentro de um conjunto de dados que resume os resultados dos pacientes e os valores de CT (limiar de ciclo, da sigla em inglês "cycle threshold", um proxy para a carga viral) por hospital, com uma linha de Totais na parte inferior. A saída é salva como o objeto `table`.
