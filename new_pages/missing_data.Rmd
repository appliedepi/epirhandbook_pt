...
3) Realizar a plotagem da quantidade de campos em branco ao longo do tempo
4) Modificar como `NA` é mostrado nas plotagens
...
No R, campos em branco são representados por um valor especial e reservado - `NA`. Notar que é escrito *sem* as aspas simples. "NA" é diferente, sendo apenas um caractere normal no R (além de ser parte da letra de Hey Jude dos Beatles).
...
Para avaliar se é Null, é só utilizar a função `is.null()`, e conversões podem ser feitas com a função `as.null()`.  
...
Abaixo são elencadas funções úteis do **base** R para avaliar e trabalhar com campos em branco:
...
Esta função, se aplicada em um conjunto de dados, irá remover linhas com *qualquer* campo em branco. Esta função também é do pacote **base** R.  
...
Esta é uma função do pacote **tidyr**, que é útil em um [pipeline de limpeza de dados][Limpando os dados e funções essenciais]. Se executada com os parênteses vazios, ela remove linhas com *qualquer* um dos campos esteja em brancos. Se o nome das colunas são especificados em parênteses, linhas com campos em branco apenas nestas colunas serão excluídas. Você pode também utilizar a sintaxe do "tidyselect" para especificar as colunas.
...
* Você pode adicionar um nome de coluna (sem as aspas) ao atributo `facet = ` para ver a plotagem por grupos
...
Aqui, os dados são canalizados `%>%` para a função. O atributo `facet = ` também é utilizado para dividir os dados.
...
Estas colunas "shadow" podem ser utilizadas para plotar a proporção de campos em branco, em relação à qualquer outra coluna.

Por exemplo, a plotagem abaixo mostra a proporção de campos em branco na coluna `days_onset_hosp` (número de dias entre o início dos sintomas e a hospitalização), de acordo com o campo em `date_hospitalisation`. Essencialmente, você está plotando a densidade da coluna no eixo x, mas estratificando os resultados (`color = `) pela coluna "shadow" de interesse. Esta análise funciona melhor se o eixo x é uma coluna numérica ou com dados cronológicos.
...
ggplot(data = shadowed_linelist,          # banco de dados com as colunas "shadow"
  ...
  geom_density()                          # plotagem das curvas de densidade
```
...
Uma forma alternativa para plotar a proporção de campos em branco em uma coluna de acordo com o tempo é mostrada abaixo. Esta forma *não* involve o pacote **naniar**. Este exemplo mostra a porcentagem de observações semanais que estão em branco.
...
2) Realize a plotagem da proporção de campos em brancos como uma linha, utilizando o `ggplot()`  
...
Então nós plotamos as proporções dos campos em branco como uma linha, de acordo com a semana. A página sobre [básico do ggplot] pode ser utilizada se você não tiver familiaridade com o pacote **ggplot2** de plotagem.
...
O objeto `linelist` original possuí ` nrow(linelist)` linhas. O número ajustado de linhas, após exclusão das que possuíam campos em branco, é mostrado abaixo:
...
Frequentemente, é sábio mostrar a quantidade de valores excluídos de uma plotagem em uma legenda. Abaixo é um exemplo:
...
* Observe que, caso colunas múltiplas contribuam para a exclusão dos valores do gráfico (ex.: age ou sex se estes interferirem na plotagem), então você precisa filtrar por estas colunas assim como calcular o número de registros não mostrados de forma correta.
...
Se a sua coluna de interesse for da classe factors, utilize a função `fct_explicit_na()`, do pacote **forcats**, para converter os valores do tipo `NA` para valores do tipo caractere. Veja mais detalhes na página [Factors]. Por padrão, o novo valor atribuído é "(Missing)", mas pode ser ajustado através do argumento `na_level =`.
...
1) Ao remover todas as observações com campos em branco, ou variáveis com uma quantidade elevada de dados em branco, você pode reduzir seu poder de amostra ou capacidade para realizar algumas análises. Por exemplo, como descobrimos anteriormente, apenas uma pequena fração das observações no nosso banco de dados no linelist não possuí campos em branco em todas as variáveis. Se nós removessemos a maioria dessas linhas, estaríamos perdendo muita informação! Também vimos que boa parte das nossas variáveis possuí alguma quantidade de dados em branco -- assim, para boa parte da análise, provavelmente não é razoável excluir todas as variáveis que possuem muitos campos vazios.
