...
-   Se utilizar qualquer função `adorn_*()` para tabelas criadas com outra função além do `tabyl()`, você pode especificar a(s) coluna(s) para aplicar o "adorn", como em `adorn_percentage(,,,c(cases,deaths))` (onde a porcentagem será adicionada somente no quarto argumento d análise). Como a sintaxe não é simples, considere utilizar a função `summarise()` em seu lugar.\
