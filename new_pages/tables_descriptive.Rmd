...
Esta página demonstra o uso dos pacotes **janitor**, **dplyr**, **gtsummary**, **rstatix**, e **base** R para gerar um resumo dos dados e criar tabelas com estatísticas descritivas.
...
-   Use a função `table()` do **base** R se você não tiver acesso aos pacotes citados acima
...
O código abaixo realiza o carregamento dos pacotes necessários para a análise dos dados. Neste manual, enfatizamos o uso da função `p_load()`, do **pacman**, que instala os pacotes, caso não estejam instalados, *e* os carrega no R para utilização. Também é possível carregar pacotes instalados utilizando a função `library()`, do **base** R. Para mais informações sobre os pacotes do R, veja a página [Básico do R].
...
As primeiras 50 linhas da lista são mostradas ao executar o código abaixo.
...
Você também pode usar a função `summary()`, do **base** R, para obter informações sobre o banco de dados inteiro, mas os resultados obtidos podem ser mais difíceis de visualizar do que utilizando o **skimr**. O resultado da análise com `summary()` não é mostrado abaixo, visando poupar espaço na página.
...
```{r}
summary(linelist$age_years)[[2]] # obtém apenas o resultado da análise no index [2]
# alternativa equivalente ao indexador [2], utilizando o nome do campo:
# summary(linelist$age_years)[["1st Qu."]]  
```

Você pode obter estatísticas individuais com outras funções do **base** R, como `max()`, `min()`, `median()`, `mean()`, `quantile()`, `sd()`, e `range()`. Veja a página [Básico do R] para uma lista completa.
...
Você pode usar a função `get_summary_stats()`, do **rstatix**, para obter o resumo estatístico *em formato de tabela*. Isso pode ser útil na execução de comandos posteriores ou na plotagem dos números. Veja a página [Testes estatísticos simples] para mais detalhes do pacote **rstatix** e suas funções.
...
Se a coluna for da classe Factor e apenas alguns níveis dessa classe estiverem presentes em seus dados, todos os níveis desta classe serão mostrados na tabela de análise. Você pode omitir essa característica ao usar o atributo `show_missing_levels = FALSE`. Leia mais na página [Factors].
...
O código abaixo modifica uma tabela de dados cruzados de forma que as quantidades absolutas e os percentuais são mostrados.
...
Você pode utilizar as funções `adorn_*()`, do pacote **janitor**, em outras tabelas, como as criadas pelas funções `summarise()` e `count()` do pacote **dplyr**, ou `table()` do **base** R. Simplesmente aplique a tabela gerada à função desejada do pacote **janitor**. Por exemplo:
...
A função `summarise()` cria uma *nova tabela com o resumo dos dados.* Se os dados *não são agrupados*, esta função gera uma tabela de dados de uma linha com os resumos estatísticos desejados do banco de dados inteiro. Se os dados são *agrupados*, a nova tabela terá um linha por *grupo* (veja a página [Agrupando dados]).
...
Os resultados das contagens de duas ou mais colunas usadas para agrupar os dados são gerados no formato "longo", com as contagens na coluna `n`. Veja a página sobre [Pivoting data] para aprender sobre as tabelas nos formatos "longos" e "amplos".
...
Está técnica é útil para padronizar suas tabelas/plotagens. Por exemplo, se você estiver criando figuras para diferentes sub-grupos, ou precisar criar um mesmo tipo de figura para relatórios de rotina. Em cada uma dessas circuntâncias, os valores nos dados podem variar, mas é possível definir níveis que continuam constantes.

Veja a página sobre [Factors] para mais informações.
...
summary_table <- linelist %>%                                        # inicie com o linelist, salvando os novos dados como um novo objeto
  ...
  summarise(                                                         # apenas as colunas abaixo serão geradas
    ...
    delay_max   = max(days_onset_hosp, na.rm = T),                    # tempo máximo entre ínicio dos sintomas e admissão
...
-   Use a função `round()`, do pacote **base** R, para arredondar e especificar quantidade de casas decimais\
...
O cálculo dos *percentis* e quartis no **dplyr** merece uma menção especial. Para obter os quartis, utilize a função `quantile()` com os intervalos padrões, ou especifique os valores alterando o atributo `probs =`.
...
Se você quer obter os quantis *por grupos*, é mais viável utilizar a função `group_by()` e criar novas colunas, uma vez que irá gerar dados mais claros em relação ao método acima, onde seriam obtidosresultados longos e menos úteis. Desta forma, experimente essa abordagem: crie um coluna para cada nível de quantil desejado.
...
Para somar as contagens (da coluna `n`) por grupo, você pode usar a função `summarise()` e ajustar a nova coluna para ser igual à `sum(n, na.rm=T)`. Para adicionar um elemnto condicional à essa operação, você pode selecionar, na coluna de contagem (`n`), uma parte dos dados utilizando os colchetes [ ].
...
Se você prefere sua tabela no formato "amplo", você pode transformar ela utilizando a função `pivot_wider()`, do pacote **tidyr**. Você provavelmente precisará renomear as colunas com a função `rename()`. Para mais informações, veja a página sobre [Pivoting os dados].
...
Se sua tabela consistir for feita de resumos estatísticos como `median()`, `mean()`, etc, a função`adorn_totals()` utilizada acima *não* será suficiente. Assim, para obter o resumo estatístico de todo o banco de dados, você precisa calcular eles com uma função `summarise()` separada e, então, conectar os resultados à tabela resumo inicial. Para fazer essa conexão, você pode usar a função `bind_rows()`, do pacote **dplyr**, descrito na página [Unindo dados]. Abaixo está um exemplo:
...
Esta tabela está no formato "longo", que pode ser o desejado. *Opcionalmente*, você pode *mudar* essa tabela para o formato *amplo*, de forma a torná-la mais fácil de interpretar. Veja a seção acima sobre como transformar a tabela para o formato "amplo", ou na página [Pivoting dados]. Você também pode adicionar mais colunas, e ajustá-las de forma agradável. Segue o código:
...
table_long %>% 
  ...
  ########################
  mutate(hospital = replace_na(hospital, "Total")) %>% 
  pivot_wider(                                         # Mude de "longo" para "amplo"
...
table <- linelist %>% 
  # filtros
  ########
  #filter(!is.na(outcome) & hospital != "Missing") %>%  # Excluí casos com campos em branco nas colunas outcome ou hospital
  ...
  ########################
  mutate(hospital = replace_na(hospital, "Total")) %>% 
  pivot_wider(                                         # Transforme de longo para amplo
...
Você também pode adicionar os resultados dos testes estatísticos nas tabelas geradas pelo **gtsummary**. Este processo está decrito na seção do **gtsummary**, da página [testes estatísticos simples](#stats_gt).
...
-   `type = all_continuous() ~ "continuous2"` Permite a realização de estatísticas em mais de uma linha ("multi-line") por variável, como mostrado em seção posterior
...
linelist %>% 
  ...
  tbl_summary(     
    ...
    type   = all_categorical() ~ "categorical",                 # force todos os níveis de colunas categóricas a serem mostradas
...
### Estatísticas "multi-line" para variáveis contínuas {.unnumbered}

Se você quer obter múltiplas linhas de estatísticas para variáveis contínuas, é possível indicar isto ao ajustar o atributo `type =` para "continuous2". É possívelcombinar todos os elementos mostrados anteriormente em uma tabela, ao escolher quais estatísticas quer mostrar. Para fazer isso, é necessário 'dizer' para a função que quer a tabela de volta ao inserir seu tipo como "continous2". A quantidade de campos em braco é mostrado como "Unknown".
...
<<<<<<< HEAD
Existem diversas formas de modificar essas tabelas, como adicionar valores do p, ajustar cores e títulos, etc. Muitas destas modificações estão descritas na documentação (digite `?tbl_summary` no Console), e algumas destas são mostradas na seção sobre [testes estatísticos](https://epirhandbook.com/simple-statistical-tests.html).
=======
Existem diversas formas de modificar essas tabelas, incluindo adicionar valores do p, ajustar cores e títulos, etc. Muitas destas modificações estão descritas na documentação (digite `?tbl_summary` no Console), e algumas destas são mostradas na seção sobre [testes estatísticos](https://epirhandbook.com/simple-statistical-tests.html).
>>>>>>> 548dffd76c08ab218297ba573041031d1ddd0d2a
...
[***DICA:*** É possível usar o `%$%` do pacote **magrittr** para remover a necessidade de repetir a quantidade de vezes que o quadro de dados é chamado dentro das funções do pacote **base**. Por exemplo, o código abaixo poderia ser reescrito como `linelist %$% table(outcome, useNA = "always")`]{style="color: darkgreen;"}
...
Para obter proporções, utilize a tabela acima na função `prop.table()`. Use o argumento `margins =` para especificar caso você queira que as proporções sejam calculadas das linhas (1), colunas (2), ou da tabela inteira (3). No código abaixo, canalizamos ("pipe") a tabela para a função `round()`, do **base** R, especificando 2 dígitos.
...
Converter um objeto `table()` diretamente para um quadro de dados não é simples. Uma abordagem é mostrada abaixo:
...
3)  Utilize a tabela na função do **base** R `as.data.frame.matrix()`\
4)  Transforme a tabela utilizando a função do pacote **tibble,** `rownames_to_column()`, especificando o nome da primeira coluna\
5)  Exporte, Visualize, ou exporte como desejado. Neste exemplo, nós utilizamos a função `flextable()`, do pacote **flextable**, como descrito na página [Tabelas para apresentação]. Isto irá exportar a tabela para o RStudio viewer como uma linda imagem HTML.
