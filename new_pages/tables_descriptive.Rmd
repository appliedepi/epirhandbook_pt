...
pacman::p_load(
  ...
  flextable     # converte tabelas para imagens 'bonitas'
  )
```
...
O pacote **janitor** contém a função `tabyl()`, que produz tabulações simples e tabulações cruzadas, as quais podem ser modificadas com funções auxiliares para mostrarem porcentagens, proporções, contagens, etc.

Abaixo, nós utilizamos as funções do **janitor** no banco de dados `linelist,` criado anteriormente, e visualizamos o resultado da análise. Se necessário, é possível salvar as tabelas geradas utilizando o operador `<-`, e atribuir as tabelas a um novo objeto no R.

### Básico do tabyl {.unnumbered}

O uso padrão do `tabyl()` em uma coluna específica produz gera os valores únicos, contagens, e proporções, em porcentagem, destes valores na coluna analisada. Como as proporções podem ter muitos dígitos, você pode ajustar o número de casas decimais com a função `adorn_rounding()`, descrita abaixo.

```{r}
linelist %>% tabyl(age_cat)
```

Como você pode observar acima, se existirem campos em branco na coluna, eles são mostrados em uma linha chamada `<NA>`. Você pode omitir estes campos com o atributo `show_na = FALSE`. Se não existirem campos em branco, essa linha não irá aparecer. Se estes campos existirem, todas as proporções são geradas como 'brutas' (coluna 'percent' gerada, onde no denominador está incluso a quantidade de `NA`) e 'válidas' (coluna 'valid_percent', onde a quantidade de campos `NA` não é considerada no cálculo).

Se a coluna é for da classe Factor e apenas alguns níveis dessa classe estiverem presentes em seus dados, todos os níveis desta classe serão mostrados na tabela de análise. Você pode omitir essa característica ao usar o atributo `show_missing_levels = FALSE`. Leia mais na página [Factors].

### Tabulação cruzada (tabela de contingência) {.unnumbered}

Contagens de tabulações cruzadas são obtidas ao adicionar uma ou mais colunas adicionais dentro da função `tabyl()`. Observe que agora apenas os números absolutos são gerados - proporções e porcentagens podem ser adicionadas à análise com etapas adicionais mostradas abaixo.

```{r}
linelist %>% tabyl(age_cat, gender)
```

### "Adornando" o tabyl {#tbl_adorn .unnumbered}

Use as funções "adorn" do **janitor** para adicionar ao resultado da análise as colunas de números absolutos ou converter para proporções, percentuais, ou ajustar o formato de gerado. Com frequência você vai aplicar essas funções nas análises geradas pelo tabyl.

+--------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Função                   | Resultado                                                                                                                                                                                                                 |
+==========================+===========================================================================================================================================================================================================================+
| `adorn_totals()`         | Adiciona a quantidade absoluta da contagem (`where =` "row", "col", or "both"). "row": linhas; "col": colunas; "both": ambas. Atribua ao campo `name =` o nome "Total".                                                   |
+--------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `adorn_percentages()`    | Converte números absolutos para proporções, escolhendo o `denominator =` "row", "col", or "all".                                                                                                                          |
+--------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+--------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `adorn_rounding()`       | Arredonde as proporções utilizando `digits =` nº de casas decimais. Para arredondar percentuais, utilize a função `adorn_pct_formatting()` com o n° de casas decimais em `digits =`.                                      |
+--------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `adorn_ns()`             | Adicione uma coluna de números absolutos em uma tabela de proporções ou porcentagens. Utilize o atributo `position =` "rear" para mostrar os números em parênteses, ou "front" para colocar os percentuais em parênteses. |
+--------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `adorn_title()`          | Adiciona texto à tabela gerada através dos argumentos `row_name =` e/ou `col_name =`                                                                                                                                      |
+--------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
```{r}
linelist %>%                                  
  tabyl(age_cat, gender) %>%                  # contagens absolutas por idade e gênero
  adorn_totals(where = "row") %>%             # adiciona a contagem absoluta
  adorn_percentages(denominator = "row") %>%  # converte as contagens para proporções
  adorn_pct_formatting(digits = 1)            # converte as proporções para porcentagens
```

Uma tabela de dados cruzados ajustada de forma que as quantidades absolutas e percentuais são calculados.

```{r}
linelist %>%                                  # fonte dos dados
  tabyl(age_cat, gender) %>%                  # geração da tabela cruzada
  adorn_totals(where = "row") %>%             # mostra as contagens absolutas
  adorn_percentages(denominator = "col") %>%  # gera proporções
  adorn_pct_formatting() %>%                  # gera os percentuais
  adorn_ns(position = "front") %>%            # mostrar como: "contagem (porcentagem)"
  adorn_title(                                # ajusta os títulos das colunas e linhas
