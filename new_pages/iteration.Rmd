...
3)  O **contêiner** para os resultados (opcional)

A sintaxe básica é: `for (item em sequência) {fazer operações usando item}`. Observe os parênteses e os colchetes. Os resultados podem ser impressos no console ou armazenados em um objeto R container.
...
-   O *vetor* pode ser de valores de caracteres, nomes de colunas ou talvez uma sequência de números - esses são os valores que mudarão a cada iteração. Você pode usá-los nas operações do *loop for* usando o termo "item".
...
Para fins de exemplo, incluímos o código de operações dentro do *loop for*, que é executado para cada valor na sequência. Neste código, os valores de sequência (nomes das colunas) são usados para *indexar* (subconjunto) `linelist`, um de cada vez. Conforme ensinado na página \[R basics\], colchetes duplos `[[ ]]` são usados para subconjunto. A coluna resultante é passada para `is.na()`, então para `sum()` para produzir o número de valores na coluna que estão faltando. O resultado é impresso no console - um número para cada coluna.
...
No exemplo abaixo, iteramos em cada linha na `linelist`. Os valores `gender` e `age` de cada linha são colados e armazenados no vetor de caracteres do contêiner `cases_demographics`. Observe como também usamos a indexação `[[i]]` para salvar a saída do loop na posição correta no vetor "container".
...
É mais eficiente criar o container para os resultados *antes* mesmo de iniciar o *loop*. Na prática, isso significa criar um vetor, data frame ou lista vazio. Estes podem ser criados com as funções `vector()` para vetores ou listas, ou com `matrix()` e `data.frame()` para um data frame.
...
Observe que para printar de dentro de um *loop for* você provavelmente precisará envolver explicitamente a função `print()`.

Neste exemplo abaixo, a sequência é um vetor de caracteres explícito, que é usado para acessar o subconjunto da linelist por hospital. Os resultados não são armazenados em um container, mas são impressos no console com a função `print()`.
...
Para juntar todos os três componentes (contêiner, sequência e operações), vamos tentar traçar uma epicurva para cada hospital (consulte a página em \[Epidemic curves\]).

Podemos fazer uma bela epicurva de *todos* os casos por gênero usando o pacote **incidence2** conforme abaixo:

```{r, warning=F, message=F}
# criando o objeto 'incidence'
...
# plotando a epicurva
plot(outbreak,                       # nome do objeto de incidência
     fill = "gender",                # barras de cores por sexo
     color = "black",                # cor do contorno das barras
     title = "Outbreak of ALL cases" # título
     )
```
...
-   O objeto de incidência é criado na linelist filtrada
...
Um loop com muitas iterações pode ser executado por muitos minutos ou até horas. Assim, pode ser útil imprimir o progresso no console R. A instrução `if` abaixo pode ser colocada *dentro* das operações de loop para exibir a cada 100 números. Basta ajustá-lo para que `i` seja o "item" em seu loop.
...
for (i in seq_len(nrow(linelist))){
  ...
  # print progress
  if(i %% 100==0){    # O operador %% é o resto da operação
    print(i)
...
Tenha em mente que o **purrr** é uma *ferramenta de programação funcional*. Ou seja, as operações que devem ser aplicadas iterativamente são agrupadas em *funções*. Consulte a página \[Writing functions\] para aprender a escrever suas próprias funções.
...
Lembre-se da página \[Import and export\] que quando usado em arquivos do Excel, `import()` pode aceitar o argumento `which =` especificando a aba a ser importada. Dentro da função `.f` `import()`, fornecemos `which = .x`, cujo valor mudará a cada iteração através do vetor `sheet_names` - primeiro "Central Hospital" ("Hospital Central"), depois "Military Hospital" ("Hospital Militar") etc.
...
Salvamos a lista de saída como `combined`.
...
-   O caminho do arquivo fornecido para `export()` é dinâmico através do uso de `str_glue()` (consulte a página \[Characters and strings\]):
...
Lembre-se da página em \[Simple statistical tests\] que `t.test()` pode receber entradas em um formato de fórmula, como `t.test(coluna numérica ~ coluna binária)`. Neste exemplo, fazemos o seguinte:
...
Nota: Lembre-se que se você deseja aplicar uma função a apenas certas colunas em um data frame, você também pode simplesmente usar `mutate()` e `across()`, conforme explicado na página \[Cleaning data and core functions\]. Abaixo está um exemplo de aplicação de `as.character()` apenas para as colunas "age". Observe o posicionamento dos parênteses e vírgulas.
...
Para extrair elementos da lista por nome ou por posição, você pode usar colchetes `[[ ]]` conforme descrito na página \[R basics\]. Abaixo usamos colchetes duplos para indexar a lista `t.tests_results` e exibir o primeiro elemento que é o resultado do teste t em `age`.
...
Indexe níveis mais profundos especificando os níveis adicionais com vírgulas. O código abaixo extrai o elemento chamado "p.value" da lista `age` dentro da lista `t.test_results`. Você também pode usar números em vez de nomes de caracteres.
