...
Esta página aborda sobre como obter ajuda postando um problema no Github ou postando um exemplo reprodutível ("reprex") em um fórum online.
...
Veja abaixo a página do Github para o pacote **incidence2** (usado para fazer curvas epidêmicas). Você pode ver a guia "Problemas" destacada em amarelo. Você pode ver que existem 5 questões em aberto.

```{r, warning=F, message=F, echo=F}
knitr::include_graphics(here::here("images", "errors_Github_issues.png"))
```

Na guia Problemas você pode ver os problemas em aberto. Revise-os para garantir que seu problema não foi resolvido ainda. Você pode abrir um novo problema clicando no botão verde à direita. Para fazer isso, será necessário ter uma conta Github.
...
*Além disso, certifique-se de não postar nenhum dado sensível com o reprex!* Você pode criar frames de dados de exemplo ou usar um dos frames de dados embutidos no R (insira `data ()` para abrir uma lista desses conjuntos de dados).
...
* Se você definir `session_info = TRUE`, a saída de `sessioninfo ::session_info()` incluírá suas versões de pacote R e R
* Você pode fornecer um diretório de trabalho para `wd =`
* Você pode ler mais sobre os argumentos e possíveis variações em [documentação]() ou inserindo `?Reprex`
