...
ggtree(tree, layout="circular", branch.length='none') %<+% sample_data + # the %<+% is used to add your dataframe with sample data to the tree
  ...
  scale_color_manual(name = "Sample Origin", # nome do seu esquema de cores (como aparecerá na leganda)          
                    breaks = c("Yes", "No"), # as difeerenres opções na sua variável
                   labels = c("NRCSS Belgium", "Other"), #como você que que as diferentes opções nomeadas na sua leganda, permite formatação
                 values= c("blue", "black"), #a coqye que você que que cada variável tenha          
...
Este trecho de código mostra o carregamento dos pacotes necessárias. Neste manual, enfatizamos `p_load()` de **pacman**, que instala o pacote se necessário *e* o carrega para utilização. Você também pode carregar os pacotes instalados com `library()` do R **base**. Veja a página em [R básico] para mais informações sobre os pacotes R.
...
Também nos certificamos de que a primeira coluna em nosso quadro de dados "Sample_data" seja "Sample_ID". Olhamos os nomes das colunas de nosso quadro de dados utilizando `colnames()` dm R **base**.

```{r}
colnames(sample_data)   
```

Olhamos para os `Sample_IDs` no quadro de dados para ter certeza de que a formatação é a mesma do que na `tip.label` (por exemplo, todas as letras são maiúsculas, sem sublinhados extras `_' entre letras e números, etc.)
...
Após a inspeção, podemos ver que o formato do `Sample_ID` no quadro de dados corresponde ao formato dos nomes das amostras nas `tip.labels`. Estas não precisam ser classificadas na mesma ordem para serem combinadas.
...
O operador **%<+%*** é utilizado para conectar o quadro de dados `sample_data` ao arquivo `tree`.
...
ggtree(tree, layout = "circular", branch.length = 'none') %<+% sample_data + # %<+% adds dataframe with sample data to tree
  aes(color = I(Belgium))+                       # colore os ramos de acordo com a variável no seu dataframe c
  scale_color_manual(
    ...
    labels = c("NRCSS Belgium", "Other"),        # como você quer que as diferenres opções sejam nomeadas na sua legenda, permite formatação 
    values = c("blue", "black"),                  # a cor que você atribuir à variável
    na.value = "black") +                        # colore valores NA values como preto também
...
sub_tree2 <- tree_subset(
  tree,
  "S17BD07692",
  levels_back = 9) #  levels_back define quantos nós quer retroceder da ponra da amostra  
```
...
no objeto `p1` e mudar o nó 37 para o topo e o nó 39 para o fundo. Armazenamos a árvore invertida em um novo objeto `p3`.
