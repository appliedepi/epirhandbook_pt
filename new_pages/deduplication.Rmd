...
Esta página abrange as seguintes técnicas de eliminação de duplicatas (ou deduplicação):  

1. Identificando e removendo linhas duplicadas  
2. "Fatiar" linhas para manter apenas algumas linhas (por exemplo, mín. ou máx.) de cada grupo de linhas  
3. "Acumulação" ou combinação de valores de várias linhas em uma linha  
...
pacman :: p_load(
  ...
  janitor, # função para revisar duplicatas
...
* As duas segundas linhas são duplicadas, em todas as colunas *exceto para `recordID`*  
...
Esta seção descreve como revisar e remover linhas duplicadas em um data frame. Também mostra como lidar com elementos duplicados em um vetor.  


<!-- ======================================================= -->
### Examinar linhas duplicadas {.unnumbered}  


Para revisar rapidamente as linhas que têm duplicatas, você pode usar `get_dupes()` do pacote **janitor**. *Por padrão*, todas as colunas são consideradas quando as duplicatas são avaliadas - as linhas retornadas pela função são 100% duplicadas considerando os valores em *todas* as colunas.  
...
obs %>% 
  janitor::get_dupes(name, purpose) %>% 
  DT::datatable(pop, rownames = FALSE, options = list(pageLength = 7, scrollX = T), class = 'white-space: nowrap')
```
...
Para manter apenas linhas exclusivas de um data frame, use `distinct()` de **dplyr** (conforme demonstrado na página [Limpeza de dados e funções principais]). As linhas duplicadas são removidas de forma que apenas a primeira dessas linhas seja mantida. Por padrão, "primeiro" significa o maior `número da linha` (ordem das linhas de cima para baixo). Apenas linhas exclusivas permanecem.  
...
obs %>% 
  distinct(across(-recordID), # reduz o data frame para apenas linhas únicas (mantém a primeira de todas as duplicatas)
           .keep_all = TRUE)
...
obs %>% 
  distinct(across(-recordID), # reduz o data frame para apenas linhas únicas (mantém a primeira de todas as duplicatas)
           .keep_all = TRUE) %>%
  DT::datatable(pop, rownames = FALSE, options = list(pageLength = 6, scrollX = T), class = 'white-space: nowrap')
```
...
**Deduplicar com base em colunas específicas**  

Você também pode especificar colunas para serem a base para eliminação de duplicação. Dessa forma, a eliminação de duplicação se aplica apenas a linhas duplicadas nas colunas especificadas. A menos que você defina `.keep_all = TRUE`, todas as colunas não mencionadas serão descartadas.  

No exemplo abaixo, a eliminação de duplicação se aplica apenas a linhas com valores idênticos para as colunas `name` e `purpose`. Assim, "brian" tem apenas 2 linhas em vez de 3 - seu *primeiro* encontro de "contato" e seu único encontro de "caso". Para ajustar para que o *último* encontro de brian de cada propósito seja mantido, veja a aba Fatiar dentro de grupos.  
...
<!-- ======================================================= -->
### Deduplicar elementos em um vetor {.unnumbered}  


A função `duplicated()` do **base** R avaliará um vetor (coluna) e retornará um vetor lógico de mesmo comprimento (VERDADEIRO/FALSO). Na primeira vez que um valor aparecer, ele retornará FALSE (não uma duplicata) e, nas próximas vezes em que esse valor aparecer, ele retornará TRUE. Observe como `NA` é tratado da mesma forma que qualquer outro valor.    
...
Para retornar apenas os elementos exclusivos, use `unique()` de **base** R. Para remover `NA`s da saída, aninhe `na.omit()` dentro de `unique()`.  
...
<!-- ======================================================= -->
### Usando **base** R {.unnumbered}

**Para retornar linhas duplicadas**  

No **base** R, você também pode ver quais linhas são 100% duplicadas em um data frame `df` com o comando `duplicated(df)` (retorna um vetor lógico das linhas).  
...
**Para retornar linhas que são duplicatas de apenas determinadas colunas**  
...
Para "fatiar" (*slice*) um data frame para aplicar um filtro nas linhas por número/posição de linha. Isso se torna particularmente útil se você tiver várias linhas por grupo funcional (por exemplo, por "pessoa") e quiser manter apenas uma ou algumas delas. 
...
```{r}
obs %>% slice_max(encontro, n = 1) # retorna linhas com o maior número de encontro
```
...
obs %>% 
  ...
  slice_max(date, # mantém linha por grupo com valor máximo de data 
            n = 1, # mantém apenas a linha mais alta
            with_ties = F) %>% # se houver empate (de data), pegue a primeira linha
  DT::datatable(pop, rownames = FALSE, options = list(pageLength = 8, scrollX = T), class = 'white-space: nowrap')
```
...
*No exemplo acima, também seria possível dividir por número de `encontro`, mas mostramos a fatia em `data` e `hora` para fins de exemplo.*  
...
<!-- ======================================================= -->
### Calcular a integridade da linha {.unnumbered} 

Crie uma coluna que contenha uma métrica para a integridade da linha (não falta). Isso pode ser útil ao decidir quais linhas priorizar em relação a outras ao desduplicar/fatiar.  
...
Isso envolve a função `rowSums()` de **base** R. Também é usado `.`, que dentro da tubulação se refere ao data frame naquele ponto na tubulação (neste caso, está sendo subconjunto com colchetes ` []`).  
...
obs %>% 
  mutate(key_completeness = rowSums(!is.na(.[,key_cols]))/length(key_cols)) %>% 
  DT::datatable(pop, rownames = FALSE, options = list(pageLength = 5, scrollX = T), class = 'white-space: nowrap')
```
...
cases_rolled <- obs %>% 
  ...
  summarise()%>%
    across(everything(), # aplica-se a todas as colunas
           ~paste0(na.omit(.x), recolher = "; "))) # função é definida que combina valores não-NA
```
...
cases_rolled <- obs %>% 
  ...
  summarise()%>%
    across(everything(), # aplica-se a todas as colunas
           ~paste0(unique(na.omit(.x)), recolher = "; "))) # função é definida que combina valores únicos não-NA
```
...
cases_rolled <- obs %>% 
  ...
  summarise()%>%
    across(everything(),                
           list(roll = ~paste0(na.omit(.x), recolher = "; ")))) # _roll é anexado aos nomes das colunas
```
...
```{r, message=FALSE, echo=F}
# exibe os dados da linelist como uma tabela
DT::datatable(case_data, rownames = FALSE, options = list(pageLength = 10, scrollX = T), class = 'white-space: nowrap')
```
...
Às vezes, você pode querer identificar duplicatas "prováveis" com base na semelhança (por exemplo, string "distance") em várias colunas, como nome, idade, sexo, data de nascimento etc. Você pode aplicar um algoritmo de correspondência probabilística para identificar duplicatas prováveis.  
