...
**Coordinate Reference Systems (CRS)** (*Sistemas de Referências de Coordenadas*) - Um CRS é um sistema baseado em coordenadas utilizado para localizar propriedades geográficas na superfície da Terra. Ele possui alguns componentes principais:  
...
pacman::p_load(
  ...
  tmap,          # para produzir mapas simples, funciona tanto para mapas interativos quanto estáticoas 
  janitor,       # para limpar nomes das colunas 
  OpenStreetMap, # para adicionar o mapa-base do OpenStreeMap ao mapa do ggplot
  spdep          # estatísticas espaciais
  ) 
...
Utilizamos o pacote **sf** (spatial features) e sua função `st_as_sf()` para criar um novo objete que chamaremos de `linelist_sf`. Esse novo objeto parece essencialmente a mesma coisa que a linelist, mas as colunas `lon` e `lat` foram designadas como colunas de coordenadas, e um sistema de referência de coordenadas (CRS) foi atribuído para quando os pontos forem mostrados. O número 4326 identifica nossas coordenadas de acordo com o [World Geodetic System 1984 (WGS84)](https://gisgeography.com/wgs84-world-geodetic-system/) - que é o padrão para coordenadas de GPS.  
...
sle_adm3 <- sle_adm3_raw %>%
  clean_names() %>% # padronizar o nome das colunas
...
Novamente, esses dados podem ser baixadados do site da HDX (link [aqui](https://data.humdata.org/dataset/sierra-leone-population)) ou pelo pacote **epirhandbook** explicado [nessa página][Download manual e dados]. Usamos `import()` para carregar o arquivo .csv. Também passamos o arquivo importado para `clean_names()` para padronizar a sintaxe dos nomes das colunas.   
...
Essa é a aparência do arquivo de população. Role para a direito para ver como cada jurisdição tem colunas com a população masculina (`male`), feminina (`female`), e a população `total`, e as subdivisões nas colunas columns por grupo de idade.

```{r message=FALSE, echo=F}
# mostra a população como a tabela
...
Novamente, baixamos os locais das unidades de saúde do HDX [aqui](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities) ou por meio das instruções na página [manual de Download e dados].   
...
```{r}
# shapefile das unidades de saúdo do OSM (open street map) 
...
Você deve estar familiarizado com o conceito de fazer *join* dos dados de uma base em outra. Vários métodos são discutidos na página [Joining data] desse manual. Um "join espacial" tem um propósito similar, mas lidando com os relacionamentos espaciais dos dados. Em vez de utilizar valores comuns às colunas para parear corretamente as observações, você pode utilizar as relações espaciais deles, tais como o fato de uma característica estar *contida* em outra, ou de um ponto ser o *vizinho mais próximo* de outro, ou contido na área de influência (*buffer*) de um certo raio de outro, etc.  
...
linelist_adm <- linelist_sf %>%
  ...
  # faça join do arquivo com as fronteiras administrativas com o da linelist, baseado na intersecção espacial
...
```{r, warning=F, message=F}
ggplot(
    data = linelist_adm,                     # inicia com linelis contendo informações das unidades administrativas 
    mapping = aes(
...
Podemos utilizar o método de join *st_nearest_feature* da função `st_join()` (do pacote **sf**) para visuaizar as unidades de saude mais próximas aos casos individuais. 
...
hf_catchment <- linelist_sf_hf %>%   # inicia com a linelist incluindo os dados das clínicas mais próximas 
  ...
  arrange(desc(case_n))              # organiza em ordem descendente 
...
Agora que já temos os nomes das unidades administrativas associadas a todos os casos (veja a seção sobre juins espaciais, acima), podemos começar a mapear as contagens dos casos por área (mapas coropléticos)
...
1) Os dados populacionais `sle_adm3_pop` são obtidos utilizando `left_join()` do pacote **dplyr** com base nos valores comuns ao longo das colunas `admin3pcod` no dataframe `case_adm3`, e coluna `adm_pcode` no dataframe `sle_adm3_pop`. Veja a página [Joining data]).  
...
Depois, criamos o objeto `map`, o qual definimos utilizando a função `openmap()` do pacote **OpenStreetMap** ([documentação](https://www.rdocumentation.org/packages/OpenStreetMap/versions/0.3.4/topics/openmap)). Passaremos os seguintes argumentos para função:We provide the following:  
...
```
Assim, nossa intenção é converter o mapa para latitude/longitude com a função `openproj()` do pacote **OpenStreetMap**. Nós passamos para a função o mapa base `map` e também o Sistema de Referência de Coordenadas (CRS) que queremos. Fazemos isso passando a string do tipo "proj.4" para a projeção WGS 1984, mas você pode passar o CRS de outras formas também. (veja [esta página](https://www.earthdatascience.org/courses/earth-analytics/spatial-data-r/understand-epsg-wkt-and-other-crs-definition-file-types/) para compreender melhor do que se trata uma string do tipo proj.4)  
...
## Mapas de calor contornados {}

Abaixo descrevemos como gerar um mapa de calor contornado de densidade dos casos, sobre ummapa base, a partir de uma linelist (uma linha por caso).  
...
A better way to visualize these neighbor relationships is by plotting them:Uma melhor forma de visualizar essas relações de vizinhança é gerando um gráfico para elas:
...
A conhecida primeira lei da geografia de Tobler diz que "todas as coisas estão relacionadas com todas as outras, mas coisas próximas estão mais relacionadas do que coisas distantes." Na epidemiologia, isso geralmente significa que o risco de um certo desfecho de saúde em uma dada região é mais similar ao de suas regiões vizinhas que daquelas regiões mais longínquas. Esse conceito foi formalizado como **autocorrelação espacial** - a propridedade estatística em que características geográficas com valores similares são agregados no espaço. Medidas estatísticas de autocorrelação espacial podem ser utilizadas para *quantificar a extensão da agregação espaciala* em seus dados, *localizar onde a agregação ocorre*, e *identificar padrões compartilhados da autocorrelação espacial* entre diferentes variáveis dos seus dados. Essa seção te dá uma visão global de algumas medidadas de autocorrelação espacial mais comuns e como calculá-las no R.
...
* Pacote Simple Features [vignette](https://cran.r-project.org/web/packages/sf/vignettes/sf1.html)
