...
Essa página mostra o uso do pacote **stringr** para avaliar e lidar com valores de caracteres ("strings").
...
Para facilitar a exibição, a maioria dos exemplos são mostrados agindo em um curto vetor definido como caracteres, no entanto, eles podem ser facilmente adaptados a uma coluna dentro de um quadro de dados.
...
pacman::p_load(
  ...
  tidyverse,  # para manipulação de dados opcional
...
Nesta página, faremos referência ocasional à `linelist`, uma lista de casos de uma simulação de epidemia de Ebola. Se você quiser acompanhar, clique para realizar o download "[clean linelist](https://github.com/appliedepi/epirhandbook_eng/raw/master/data/case_linelists/linelist_cleaned.rds)" (como arquivo .rds). Importe dados com a função `import()` do pacote **rio** (ele lida com muitos tipos de arquivo como .xlsx, .csv, .rds - veja a página \[Importar e exportar\] para detalhes).
...
```{r, eval=F}
# Exemplo de importação da lista 
...
Para combinar ou concatenar múltiplos strings em um string, sugerimos usar `str_c` do **stringr**. Se você tiver distintos valores de caracteres para combinar, basta fornecê-los como argumentos exclusivos, separados por vírgulas.
...
O argumento `collapse =` é relevante se você estiver inserindo múltiplos *vetores* como argumentos para `str_c()`. É usado para separar os elementos do que seria um vetor de *saída*, de forma que o vetor de saída tenha apenas um elemento de caractere longo.
...
Use `str_glue()` para inserir um código R dinâmico em um string. Esta é uma função muito útil para criar legendas dinâmicas de plotagens, conforme demonstrado abaixo.
...
Um simples exemplo da plotagem de legenda dinâmica:
...
Às vezes, é útil extrair dados de uma tabela de dados e colá-los juntos na sequência. Abaixo está um exemplo de tabela de dados (data frame). Vamos usá-lo para fazer uma declaração resumida sobre as jurisdições e as contagens de casos novos e totais.
...
Use `str_glue_data()`, que é feito especialmente para obter dados de linhas da tabela de dados:

```{r}
caso_tabela %>% 
  str_glue_data("{zone}: {novos_cases} ({total_casos} total casos)")
```

**Combine strings em linhas**

Se você estiver tentando "acumular" valores em uma coluna de quadro de dados, por exemplo, combinar valores de várias linhas em apenas uma linha, colando-os juntos com um separador, consulte a seção da página \[Eliminação de duplicação\] em ["acumulando" valores](#str_rollup).

**Tabela de dados para uma linha**

Você pode fazer a instrução aparecer em uma linha usando `str_c()` (especificando a tabela de dados e os nomes das colunas) e fornecendo os argumentos `sep =` e `collapse =`.

```{r}
str_c(caso_tabela$zone, caso_tabela$new_cases, sep = " = ", collapse = ";  ")
```
...
```{r}
str_detect(string = "professor de escola primária", pattern = "ensino")
```

O argumento `negate =` pode ser incluído e definido como `TRUE` se você quiser saber se o padrão NÃO está presente.

```{r}
str_detect(string = "professor de escola primária", pattern = "ensino", negate = TRUE)
```
...
```{r}
str_detect(string = "Professor", pattern = regex("ensino", ignore_case = T))
```
...
# Detecta a presença do padrão "ensino" em cada string - a saída é um vetor TRUE / FALSE
str_detect(occupations, "ensino")
```
...
```{r}
sum(str_detect(occupations, "ensino"))
```
...
```{r}
sum(str_detect(string = occupations, pattern = "ensino|professor|tutor"))
```
...
as.numeric(gsub(pattern = ",",                # encontra vírgulas commas     
                replacement = ".",            # substitui por pontos
                x = gsub("\\.", "", lengths)  # vetor com outros períodos removidos (períodos de escape)
                )
...
df <- df %>% 
  mutate(is_educator = case_when(
    ...
    str_detect(occupations,
               regex("ensino|prof|tutor|university",
...
df <- df %>% 
  ...
  mutate(is_educator = case_when(
    ...
    # a coluna de ocupação deve atender a 2 critérios para ser atribuído "Educator":
    # deve ter um termo de pesquisa E NÃO qualquer termo de exclusão
    
    # Deve conter um termo de pesquisa
    str_detect(occupations,
               regex("ensino|prof|tutor|university", ignore_case = T)) &              
...
`str_extract_all()` retorna os próprios padrões de correspondência, o que é mais útil quando você oferece vários padrões por meio de condições "OU". Por exemplo, procurando no vetor string de ocupações (consulte a aba anterior) "ensino", "prof" ou "tutor".
...
```{r}
str_extract(occupations, "ensino|prof|tutor")
```
