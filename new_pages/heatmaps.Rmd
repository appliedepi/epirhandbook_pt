...
Este pedaço de código mostra o carregamento de pacotes necessários para as análises. Neste manual, enfatizamos `p_load()` do **pacman**, que instala o pacote se necessário *e* o carrega para uso. Você também pode carregar pacotes instalados com `library()` do R **base**. Veja a página em [R basics] para mais informações sobre pacotes R.  
...
```{r, warning=F, message=F, echo=F}
# Crie um quadro de dados de cada semana-estabelecimento possível
Expandido_weeks <- agg_weeks%>% 
  mutate(week = as.factor (week))%>% # converte a data em um fator para que expand() funcione corretamente
  tidyr::expand(., week, location_name)%>% # expand data frame para incluir todas as combinações possíveis de semana-estabelecimento
                                             # Nota: "." representa o conjunto de dados naquele momento no encadeamento do código (pipe)
...
agg_weeks <- agg_weeks %>%      
  right_join(extended_weeks)%>% # Certifique-se de que todas as combinações possíveis de estabelecimento-semana apareçam nos dados
