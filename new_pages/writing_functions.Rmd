...
Este pedaço de código mostra o carregamento de pacotes necessários para as análises. Neste manual, enfatizamos `p_load()` de **pacman**, que instala o pacote, se necessário, *e* carrega ele para ser utilizado. Você também pode carregar pacotes instalados com  `library()` de **base** R. Veja em [R básico] para mais informações sobre pacotes R.  
...
pacman::p_load(
  ...
  gtsummary,    # Estatísticas sumárias (de resumo) e testes
  ...
  with(my_data,
       rbind( # Vincula os resultados das duas funções seguintes por linha
        # Tabula a variável de interesse: fornece apenas números
          table(my_data[[var_to_tab]], useNA = "não"),
...
table_indic_all <- indic_1 %>% 
  dplyr::left_join(indic_2, by = "provincia") %>% 
        left_join(indic_3, by = "provincia")
...
print_indic_prov <-  function(table_used, prov){
  ...
  # Primeiro, transforme um pouco o quadro de dados para facilitar a impressão
 indic_prov <- table_used %>%
    filter(province==prov) %>%
    pivot_longer(names_to = "Indicadores", cols = 2:4) %>% 
   mutate( indic_label = factor(Indicadores,
   levels= c("atraso_inicio_hosp","perc_recuperacao","mediana_idade"),
...
Se você quiser saber como fazer referência a *nomes de coluna* fornecidos em seu código como argumentos, leia  [tidyverse programming guidance](https://dplyr.tidyverse.org/articles/programming.html).Entre os tópicos cobertos estão *avaliação arrumada * e uso de *embrace* `{{ }}` "Colchetes duplos"
...
- Pode ser mais do que recomendado introduzir uma verificação sobre a falta de um argumento usando `ausente (argumento)`. Esta verificação simples pode retornar um valor "VERDADEIRO" ou "FALSO".
