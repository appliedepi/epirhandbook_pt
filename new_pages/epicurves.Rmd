...
2) **Crie o gráfico** com o objeto do tipo *incidence*  
...
O objeto **incidence2**, por si próprio, parece um tibble (um tipo de data frame), podendo ser exportado ou manipulado como tal.
...
```{r}
# visualize um resumo do objeto to tipo incidence
summary(epi_day)
```
...
Observe que o argumento `date_breaks = ` funciona apenas se `centre_dates = FALSE`. Forneça um valor de carácter entre aspas utilizando a sintaxe de strptime abaixo, como detalhado na página [Trabalhando com datas](#dates). Você pode utilizar `\n` para uma "nova linha".
...
Note que, se você adicionar `scale_x_date()`, boa parte da formatação das datas com `plot()` será substituída. Veja a seção do `ggplot()` sobre epicurvas e a página [dicas do ggplot](#ggplot-tips) para mais opções.
...
plot(
  ...
  labs(
    ...
    caption = stringr::str_glue(                            # legenda dinâmica - veja a página sobre caracteres e strings para mais detalhes
      "n = {central_cases} do Hospital Central
      Aparencimento dos casos entre {earliest_date} e {latest_date}. {missing_onset} casos estão sem a data de aparecimento e não foram mostrados",
...
plot(age_outbreak, fill = age_cat, title = " Paleta padrão 'vibrant' do incidence2")

# faça um gráfico com uma cor diferente da paleta
#plot(age_outbreak, fill = age_cat, col_pal = muted, title = "Paleta 'mute' do incidence2")
```

Você pode também utilizar uma das paletas do R **base** (coloque o nome da paleta *sem* as aspas).

```{r out.width = c('50%', '50%'), fig.show='hold', warning = F, message = F}
# crie o gráfico com uma paleta do R base
plot(age_outbreak, fill = age_cat, col_pal = heat.colors, title = "Paleta heat.colors do R base")

# crie um gráfico com uma paleta do R base
...
Como mencionado na página sobre [dicas do ggplot](#ggplot-tips), você pode criar suas próprias paletas utilizando `colorRampPalette()` em um vetor de cores, e especificando o número de cores que você quer em retorno. Esta é uma boa maneira de obter muitas cores de uma vez ao especificar apenas algumas.
...
Para ajustar a ordem de aparecimento dos grupos (no gráfico e na legenda), a coluna de agrupamento precisa ser da classe Factor. Veja a página sobre [Fatores](#factors) para mais informações.
...
* Ajuste o conjunto de dados - neste caso, nós iremos definir um novo banco de dados (`plot_data`) em que:
  * a coluna `gender` está definida como um fator (*Factor*), a ordem dos níveis está ajustada em `fct_relevel()`, de forma que "Other" e "Missing" vem primeiro e aparecem no topo das barras
* O objeto do tipo incidence é criado e um gráfico é criado, como mostrado anteriormente 
...
```{r, message=F, warning=F}
# MODIFICADO - hospital como fator
###############################

# carregue o pacote forcats para trabalhar com fatores
pacman::p_load(forcats)

# Converta a coluna hospital para factor, e ajuste os níveis
plot_data <- linelist %>% 
  mutate(hospital = fct_relevel(hospital, c("Missing", "Other"))) # Ajuste "Missing" e "Other" como níveis para estarem no topo
...
```{r, warning=F, message=F}
rolling_avg <- incidence(                    # crie um objeto do tipo incidence
  linelist,
  date_index = date_onset,
  interval = "week",
  groups = gender) %>% 
...
# faça o gráfico
plot(rolling_avg) # foi facetado automaticamente devido à média móvel dos grupos
```

Para aprender sobre como aplicar médias móveis de forma mais geral nos dados, veja a página deste manual sobre [Médias móveis](#moving-average).
