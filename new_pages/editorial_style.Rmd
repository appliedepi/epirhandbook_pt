...
O público em potencial para este livro é grande. Ele será certamente utilizado por pessoas muito novas no R, e também por utilizadores experientes do R que procuram as melhores práticas e dicas. Portanto, deve ser ao mesmo tempo acessível e sucinto. Por conseguinte, a nossa abordagem foi fornecer explicar o de *suficiente* no texto para que alguém muito novo no R possa aplicar o código e seguir o que o código está a fazer.  
...
* Este destina-se a ser um documento vivo - o pacotes R que são ideais para uma determinada tarefa tendem a mudar frequentemente, e acolhemos de bom grado a discussão sobre qual deles deveríamos enfatizar neste manual


### Pacotes R {.unnumbered}

**São tantas escolhas***  

Um dos aspectos mais desafiadores da aprendizagem de R é saber qual o pacote R utilizar para uma dada tarefa. É uma ocorrência comum esforçar-se demasiadamente em uma tarefa e só mais tarde perceber - ei, há um pacote R que faz tudo isso numa única linha de comando!  

Neste manual, tentamos oferecer-lhe pelo menos duas formas de completar cada tarefa: um método experimentado e validado (provavelmente em R **base*** ou **tidyverse**) e um pacote especial R que tenha sido construído para esse fim. Queremos que tenham algumas opções caso não consigam fazer o *donwload* de um determinado pacote ou que este não funcione.  
...
No texto narrativo, os nomes dos pacotes são escritos a negrito (por exemplo **dplyr***) e as funções são escritas desta forma: `mutate()`. Esforçamo-nos por ser explícitos sobre de que pacote vem uma função, quer referenciando o pacote em texto próximo ou especificando o pacote explicitamente no código como este: `dplyr::mutate()`. Pode parecer redundante, mas estamos fazendo isso de propósito.

Veja a página em [R fundamentos] para saber mais sobre pacotes e funções.   
