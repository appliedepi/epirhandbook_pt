...
A função `guess_dates()` do pacote **linelist** tenta ler uma coluna de data "bagunçada" contendo datas em muitos formatos diferentes e converte as datas para um formato padrão. Você pode [ler mais online sobre `guess_dates()`](https://www.repidemicsconsortium.org/linelist/reference/guess_dates.html). Se `guess_dates()` ainda não estiver disponível no CRAN para R 4.0.2, tente instalar via `pacman::p_load_gh("reconhub/linelist")`[^dates-1].

[^dates-1]: Recentemente o Github exige o uso e configuração dos PAT, *Personal Access Token*. Verifique a página oficial do Github sobre o assunto aqui: <https://docs.github.com/pt/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token>
