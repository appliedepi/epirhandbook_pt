...
Esse trecho de código mostra o carregamento dos pacotes necessários para esta página. Neste manual enfatizamos a função `p_load()` do **pacman**, que instala o pacote se necessário e o carrega para utilização. Tamém é possível carregar os pacotes instalados com `library()` a partir da **base** R. Para mais informações, veja a página em [R basics] sobre pacotes no R.

```{r dates_packages, warning=F, message=F}
# Confira se o pacote está instalado, instale o pacote se necessário, e carregue-o para a sessão atual

pacman::p_load(
  lubridate,  # pacote geral para manipulação e conversão de datas  
  linelist,   # possui a função de "adivinhar" datas desorganizadas
...
Você pode obter a data atual do "sistema" ou a data e hora do sistema do seu computador por meio da seguintes funções presentes na **base** do R.
...
`as.Date()` é a função presente na base do R para conversão um objeto ou coluna para a classe Data (observe a capitalização de "D").

Para o uso da função `as.Date()` é preciso que:

-   Você especifique o formato ***existente** da data como caractere bruto* ou a data de origem se fornecer datas como números (consulte a seção sobre datas do Excel)\
-   Se foi usado em uma coluna de caracteres, todos os valores de data devem ter o mesmo formato exato (se este não for o caso, tente `guess_dates()` do pacote **linelist**)

**Primeiro**, verifique a classe de sua coluna com `class()` da **base** R. Se você não tem certeza ou está confuso sobre a classe de seus dados (por exemplo, você vê "POSIXct", etc.) pode ser mais fácil primeiro converter a coluna para a classe Caractere com a função `as.character()`, e depois converter para a classe Date.
...
[***DICA:*** O argumento `format =` da função `as.Date()` não está informando ao R o formato que deseja que as datas tenham, mas sim como identificar as partes da data da forma como elas estão, *antes* de rodar o comando.]{style="color: darkgreen;"}

[***TIP:*** Certifique-se de que no argumento `format =` você usa o separador de data (por exemplo, /, -, ou espaço) que está presente em suas datas.]{style="color: darkgreen;"}

Uma vez que os valores estão na classe Date, o R os exibirá por padrão no formato padrão, que é AAAA-MM-DD.

### **lubridate** {.unnumbered}

A conversão de objetos de caracteres em datas pode ser facilitada com o uso do pacote **lubridate**. Este é um pacote **tidyverse** projetado para tornar o trabalho com datas e horários mais simples e consistente do que na **base** do R. Por essas razões, **lubridate** é frequentemente considerado o pacote padrão ouro para datas e horários, e é recomendado sempre que trabalhar com eles.
...
Se estiver usando piping, a conversão de uma coluna de caracteres para datas com **lubridate** pode ser assim:
...
Microsoft Excel armazena datas com um origem em 30 de dezembro de 1899 (Windows) ou 1 de janeiro de 1904 (Mac), dependendo do seu sistema operacional. Consulte em [Microsoft guidance](https://docs.microsoft.com/en-us/office/troubleshoot/excel/1900-and-1904-date-system) para obter mais informações.
...
A função `guess_dates()` do pacote **linelist** tenta ler uma coluna de data "bagunçada" contendo datas em muitos formatos diferentes e converte as datas para um formato padrão. Você pode [ler mais online sobre `guess_dates()`](https://www.repidemicsconsortium.org/linelist/reference/guess_dates.html). Se `guess_dates()` iainda não estiver disponível no CRAN para R 4.0.2, tente instalar via `pacman::p_load_gh("reconhub/linelist")`.

Por exemplo, o `guess_dates` um vetor dos seguintes caracteres de datas "03 Jan 2018", "07/03/1982", e "08/20/85" e os converteria para a classe Date como: `2018-01-03`, `1982-03-07`, e `1985-08-20`.
...
Alguns argumentos opcionais para `guess_dates()` que você pode incluir são:

-   `error_tolerance` - A proporção dos dados de entrada que não podem ser identificadas como datas a serem toleradas (o padrão é 0,1 ou 10%)

-   `last_date` - a última data válida (o padrão é a data atual)\

-   `first_date` - a primeira data válida. O padrão é cinquenta anos antes de last_date.

```{r eval = FALSE}
# Um exemplo usando guess_dates na coluna dater_onset
linelist <- linelist %>%                 # o conjunto de dados é chamado de linelist
  mutate(
    date_onset = linelist::guess_dates(  # a função guess_dates() do pacote "linelist"
      date_onset,
      error_tolerance = 0.1,
...
Um objeto `datetime` padrão object é formatado com a data primeiro, que é seguida por um componente de hora - por exemplo, *01 de janeiro de 2020*, *16:30*. Assim como acontece com as datas, isso pode ser formatado de muitas maneiras e vários níveis de precisão (horas, minutos, segundos) que podem ser fornecidos.
...
Converte datetime contendo apenas horas para o objeto datetime
...
Converte datetime com horas e minutos para o objeto datetime

```{r}
dmy_hm("01 January 2020 16:20")
```

Converte datetime com horas, minutos e segundos para o objeto datetime
...
Ao trabalhar com uma tabela de dados, as colunas de hora e data podem ser combinadas para criar uma coluna datetime usando `str_glue()` do pacote **stringr** e uma função apropriada do **lubridate**. Consulte a página em [Caracteres e junções] para obter detalhes sobre o **stringr**.
...
linelist <- linelist %>%
  ...
  # quando o tempo de admissão não é fornecido, atribua o tempo médio de admissão
  mutate(
    ...
    # em seguida, use ymd_hm() para converter em datetime
  mutate(
...
Se seus dados contém apenas um tempo de caractere (horas e minutos), você pode convertê-los e manipulá-los como tempos usando `strptime()` a partir da **base** do R. Por exemplo, para obter a diferença entre dois desses tempos:
...
Para saber mais sobre `case_when()` , consulte a página [Limpeza dos daos e funções principais].

<!-- ======================================================= -->

## Trabalhando com datas

`lubridate` ctambém pode ser usado para uma variedade de outras funções, como **extrair aspectos de uma data / data e hora, realizar aritmética de data ou calcular intervalos de data**
...
No contexto de uma tabela de dados, se uma das datas acima estiver faltando, a operação falhará para essa linha. Isso resultará em um `NA` iem vez de um valor numérico. nstead of a numeric value. Ao usar esta coluna para cálculos, certifique-se de definir o argumento `na.rm =` como `TRUE`. Por exemple:
...
Ajuste a exibição de data com a função `format()`da **base** do R. Esta função aceita uma cadeia de caracteres (entre aspas) especificando o formato de saída *desejado* nas abreviações de strptime "%" strptime abbreviations (a mesma sintaxe usada em `as.Date()`). Abaixo estão mostradas a maioria das abreviações comuns.
...
Consulte a página em [Grouping data] para exemplos mais abrangentes de dados de agrupamento por data. Abaixo, descrevemos resumidamente os dados de agrupamento por semanas.
...
Se você estiver interessado apenas em ajustar as datas para *exibição* por semana em um gráfico, consulte a seção nesta página sobre Exibição de data. Por exemplo, ao plotar uma epicurva, você pode formatar a exibição da data fornecendo a nomenclatura "%" do strptime desejada.Por exemplo, use "%Y-%W" ou "%Y-%U" para retornar o ano e o número da semana (dado o início da semana na segunda-feira ou no domingo, respectivamente).

### Contagens semanais {.unnumbered}

Veja a página em [Dados de agrupamento] para uma explicação completa de dados de agrupamento com `count()`, `group_by()`, e `summarise()`. Um breve exemplo está mostrado abaixo.
...
Para lidar com fusos horários, há várias funções auxiliares no lubridate que podem ser usadas para alterar o fuso horário de um objeto datetime do seu fuso horário local para um fuso horário diferente. Time zones are set by attributing a valid tz database time zone to the datetime object. Os fusos horários são definidos atribuindo um fuso horário do banco de dados tz válido ao objeto datetime. Uma lista deles pode ser encontrada aqui - se o local do qual você está usando os dados não estiver nessa lista, grandes cidades próximas no fuso horário estão disponíveis e têm a mesma finalidade.
...
## Cálculos atrasados e subsequentes

`lead()` and `lag()` são funções do pacote **dplyr** package que ajudam a encontrar valores anteriores (lead) ou subsequentes (lag) em um vetor - normalmente um vetor numérico ou de data. Isso é útil ao fazer cálculos de mudança / diferença entre unidades de tempo.
