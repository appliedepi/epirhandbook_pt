...
# Séries temporais e detecção de surtos { }  
...
O código abaixo garante que a coluna de datas está no formato apropriado.
Nesta página, iremos utilizar o pacote **tsibble** com funções `yearweek` 
que serão usadas para criar uma variável com um calendário de semanas. Existem  outras diferentes
maneiras de fazer isso (veja a página [trabalhando com datas](https://epirhandbook.com/working-with-dates.html)
para mais detalhes), entretanto, para séries temporais é melhor manter dentro de um framework apenas (**tsibble**). 

```{r clean_data}

## garante que a coluna com datas está no formato apropriado
counts$date <- as.Date(counts$date)

## cria uma variável com semanas do calendário
## ajusta definições ISO de semanas iniciando na segunda
counts <- counts %>% 
     mutate(epiweek = yearweek(date, week_start = 1))

```

### Baixe os dados climáticos {.unnumbered} 
...
**photon** é que ele fornece mais dados contextuais para quando existirem diferentes
resultados do que se busca.
...
for (i in 2002:2011) {
  ...
  ## baixe o aquivo e o armazene no diretório de trabalho atual
...
Se você baixou os dados climáticos via nosso manual, ou utilizou o código acima, agora você deve ter 10 anos de arquivos com dados climáticos ".nc" armazenados na mesma pasta do seu computador.
...
## apenas mantenha os arquivos com o nome de interesse atual
...
Uma vez que estes arquivos foram importados como o objeto `data`, nós iremos convertê-los para um quadro de dados.

```{r}
## mude para um quadro de dados
temp_data <- as_tibble(data) %>% 
  ## adicione variáveis e corrija unidades
  mutate(
    ## crie uma variável com um calendário semanal
    epiweek = tsibble::yearweek(time), 
    ## cria uma variável de data (início do calendário semanal)
...
Executar `class(counts)` mostra que, além de ser um quadro de dados organizado 
("tbl_df", "tbl", "data.frame"), ele tem propriedades adicionais de um quadro de dados 
de uma série temporal ("tbl_ts"). 

Você pode analisar rapidamente os seus dados ao utilizar o **ggplot2**. Nós vemos pela plotagem que
...
## plota um gráfico de linha de casos por semana
...
Veja a página sobre [De-duplicação](https://epirhandbook.com/de-duplication.html)
...
## obtenha um quadro de dados das linhas duplicadas
...
Através da breve inspeção acima, nós vimos que não existem campos em branco em nossos dados, mas também
...
imputar novos valores. A forma mais simples de imputação em uma série temporal é desenhar 
uma linha reta entre o último valor preenchido, e o próximo valor preenchido (entre os campos em branco).
Para fazer isso, nós iremos utilizar a funçaõ `na_interpolation()` do pacote **imputeTS**. 

Veja a página sobre [Campos em branco](https://epirhandbook.com/missing-data.html) para mais informações sobre imputação.

Outra alternativa seria calcular a média móvel, para tentar suavizar
esses problemas aparentes de notificação (veja a próxima seção, e a página sobre [médias móveis](https://epirhandbook.com/moving-averages.html)). 
...
ggplot_na_imputations(counts$case_miss, counts$case_int) + 
  ## crie uma plotagem tradicional (com eixos negros e fundo branco)
  theme_classic()
...
torná-los mais fáceis de interpretar. No nosso caso, isto não agrega muito, então
nós iremos manter os dados interpolados para mais análises.
Veja a página sobre [Médias móveis](https://epirhandbook.com/moving-averages.html) para mais detalhes.

```{r moving_averages}

## cria uma variável de média móvel (o que resolve o problema dos campos em branco)
counts <- counts %>% 
     ## cria a variável ma_4w 
     ## passa por cada linha da variável dos casos
     mutate(ma_4wk = slider::slide_dbl(case, 
                               ## para cada linha, calcula a média
                               ~ mean(.x, na.rm = TRUE),
                               ## usa as quatro semanas anteriores
                               .before = 4))

## faz uma visualização rápida da diferença
...
## counts é uma variável com dados contados ou taxas dentro do x
...
periodogram <- function(x, 
    ...
    ## garanta que não é uma classe tsibble, filtra por projeto e mantém apenas colunas de interesse
    prepare_data <- dplyr::as_tibble(x)
    
    # prepare_data <- prepare_data[prepare_data[[strata]] == j, ]
    prepare_data <- dplyr::select(prepare_data, {{counts}})
    
    ## cria uma zona intermediária de série temporal para ser capaz de utilizar no spec.pgram
    ...
    ## retorna as semanas com picos de casos
...
## plote um periodograma mostrando a peridiocidade que ocorre mais frequentemente
ggplot(data = periodo, 
...
* O círculo-tendência (a tendência dos dados no longo prazo)
...
counts %>% 
  ...
  ## extrai a informação importante do modelo
  components() %>% 
  ## gera uma plotagem
  autoplot()
...
e das semanas antes dela (chamado de lags).

Ao utilizar a função `ACF()`, nós podemos gerar um gráfico que mostra o número de linhas 
em relação aos diferentes lags. Quando o lag for 0 (x = 0), esta linha deve ser sempre 
1 pois mostra a relação entre uma observação e ela mesmo (não mostrado aqui).
A primeira linha mostrada aqui (x = 1) mostra a relação entre cada observação
e a observação antes dela (lag de 1), a segunda mostra a relação entre 
cada observação e a observação anterior a última (lag de 2) e assim por diante, até o lag de 
...
counts %>% 
  ## calcule a autocorrelação utilizando lags que cobrem anos completos
...
Você pode testar formalmente a hipótese nula de independência em uma série temporal (ex.:
ausência de autocorrelação) utilizando o teste de Ljung-Box (no pacote **stats**). 
Um valor significante de p sugere que existe uma autocorrelação nos dados.
...
<!-- ======================================================= -->
### Termos fourier {.unnumbered}

Termos fourier são os equivalentes de curvas seno e coseno. A diferença é que 
estes são ajustados com base na identificação da combinação mais apropriada das curvas para explicar 
seus dados.

O ajuste de apenas um termo fourier equivale a ajustar um seno e um coseno para o lag que ocorre mais frequentemente em seu periodograma (no nosso
...
## adicione os termos fourier utilizando as variáveis epiweek e case_int
...
pacote **trending**, pois isto permite o cálculo apropriado dos intervalos de confiança
...
A outra diferença é que nós primeiro definimos o modelo, e então `fit()` (ajustamos) ele para os 
...
model <- glm_nb_model(
  ...
  case_int ~
    ...
    epiweek +
    ## utilize os termos fourier para levar em consideração a sazonalidade
    fourier)
...
ggplot(data = observed, aes(x = epiweek)) + 
  ...
  geom_line(aes(y = estimate),
            col = "Red") + 
  ## adicione uma banda para os intervalos de predição
  ...
  geom_line(aes(y = case_int), 
            col = "black") + 
  ## faça uma plotagem tradicional (com eixos pretos e fundo branco)
  theme_classic()
...
Para avaliar o quão bem nosso modelo ajusta os dados observados,  precisamos checar os resíduos. 
...
O que vemos abaixo é que não estamos explicando toda a variação
que nós poderíamos com o modelo. Pode ser que deveríamos ajustar mais termos fourier,
e verificar a amplitude. Entretanto, para este exemplo, nós iremos deixar da forma que esta.
As plotagens mostram que nosso modelo possuí pior desempenho nos picos e baixas (quando as contagens estão
nas maiores e menores quantidades) e, assim, ele provavelmente irá subestimar 
...
## os resíduos são razoavelmente constantes através do tempo (se não: surtos? mudanças na prática?)
observed %>%
...
## H0 é que os resíduos são ruídos brancos (ex.: aleatórios)
## teste de independência
## se o valor de p for significante, então não aleatório
...
manual a seção sobre [unindo dados](https://epirhandbook.com/joining-data.html).

```{r join}

## junção à esquerda, de forma que nós teremos apenas as linhas existentes na contagem
...
Para mais sobre o pivoteamento dos dados, veja a seção sobre [pivoteando dados](https://epirhandbook.com/pivoting-data.html) neste manual. 
...
counts %>% 
  ...
  pivot_longer(
    ...
    values_to = "value") %>% 
  ## crie uma plotagem com os dados acima
  ## plote a epiweek no eixo x e os valores (contagens/temperatura) no eixo y
  ggplot(aes(x = epiweek, y = value)) + 
    ## crie uma plotagem separada para temperaturas e contagens dos casos
    ## deixe a escala do eixo y livre
    facet_grid(measure ~ ., scales = "free_y") +
    ## plote as informações como linha
    geom_line()
...
counts %>% 
  ...
  ## mostre os lags com maior associação
  arrange(-ccf) %>% 
...
A partir disso, nós vemos que um lag de 4 semanas é mais altamente correlacionado.
...
precisaríamos excluir esses campos em branco, e guardá-los como um novo conjunto de dados ao adicionar `drop_na(t2m_lag4)` 
...
Nós ajustamos uma regressão binominal negativa como anteriormente. Desta vez, adicionamos 
a variável temperatura com lag de quatro semanas.

<span style="color: orange;">**_CUIDADO:_** Observe o uso do argumento  `simulate_pi = FALSE`
dentro da função `predict()`. Isto é porque o comportamento padrão de **trending** 
...
Para investigar os termos individual, nós podemos puxar a regressão binominal negativa original
...
O que isso nos mostra é que a temperatura laggada, depois de controlar o modelo para tendência e sazonalidade, 
...
ggplot(data = observed, aes(x = epiweek)) + 
  ...
  geom_line(aes(y = estimate),
            col = "Red") + 
  ## adicione uma banda com os intervalos de predição
  geom_ribbon(aes(ymin = lower_pi, 
                  ymax = upper_pi), 
              alpha = 0.25) + 
...
Os resultados e interpretação são similares àqueles da regressão anterior,
então pode ser mais paupável continuar com o modelo mais simples sem a temperatura.
...
## os resíduos são razoavelmente constantes através do tempo (caso não: surtos? mudanças na prática?)
observed %>%
  ggplot(aes(x = epiweek, y = resid)) +
  geom_line() +
  geom_point() + 
...
## H0 é que os resíduos são uma série de ruídos brancos (ex.: aleatórios)
## teste de independência
## se o valor p for significante, então os resíduos não são aleatórios
Box.test(observed$resid, type = "Ljung-Box")
...
que possuí uma variedade de algoritmos diferentes para detecção de aberrações.
...
<span style="color: black;">**_NOTA:_** Neste exemplo, nós fingímos estar no final de setembro de 2011 ("2011 W39").</span>  
...
## defina a última data de interesse (ex.: final da previsão)
...
de linhas em nosso banco de dados, ex.: uma linha para cada semana até a data final (`end_date`) definida acima.
...
Nós precisamos redefinir nossos termos fourier - uma vez que queremos ajusta-los ao período de referência
apenas, e então prever (extrapolar) esses termos para o próximo ano.
Para fazer isso, nós precisamos combinar duas listas geradas da função `fourier()`; 
a primeira é para os dados base de referência, e a segunda preve para o
...
## defina os termos fourier (senos e cosenos) 
counts <- counts %>% 
  mutate(
    ## combine os termos fourier para semanas antes e após a data limite de 2010
    ## (termos fourier para 2011 e antes são estimados)
    fourier = rbind(
      ## obtenha termos fourier dos anos anteriores
      fourier(
      ...
      ## preveja os termos fourier para 2011 (utilizando os dados base de referência)
      fourier(
        ## mantenha apenas as linhas dentro do período de interesse
        filter(counts, 
               epiweek <= cut_off),
        ## inclua um conjunto de termos seno e coseno
        K = 1, 
...
model <- glm_nb_model(
  ...
  case_int ~
    ...
    epiweek +
    ## use os termos fourier para considerar a sazonalidade
    fourier
)
...
ggplot(data = observed, aes(x = epiweek)) + 
  ## adiciona uma linha para as estimativas do modelo
  geom_line(aes(y = estimate),
            col = "grey") + 
  ## adicione uma banda para os intervalos de predição
  geom_ribbon(aes(ymin = lower_pi, 
...
Como pode ser visto na figura abaixo, de *Hyndman et al* em ["Princípios e práticas
de previsão"](https://otexts.com/fpp3/). 
...
## nomeia cada conjunto de dados com um id único
## apenas utilize os casos antes do ano de interesse (ex.: 2011)
case_roll <- counts %>% 
  filter(epiweek < cut_off) %>% 
  ## apenas mantenha as variáveis de semana e quantidade de casos
...
forecasts <- purrr::map(unique(case_roll$.id), 
  ...
  ## apenas mantenha a janela atual sendo ajustada no modelo
  ...
  cv_cut_off <- mini_data %>% 
    ## apenas mantenha as linhas sem campos em branco
    drop_na(case_int) %>% 
  ...
  ## defina os termos fourier (sincos) 
  mini_data <- mini_data %>% 
    mutate(
    ## combine os termos fourier para semanas antes e após a data limite
    fourier = rbind(
      ## obtenha termos fourier para anos anteriores
      forecast::fourier(
...
mas incluí tendências e termos fourier (então é a favorita aqui). A regressão é utilizada
...
## defina o controle
ctrl <- list(
  ## defina qual período de tempo que quer utilizar o limiar (ex.: 2011)
  ...
  trend = TRUE,
  pThresholdTrend = 1, ## 0.05 normalmente, entretanto 1 é recomendado no método melhorado (ex.: sempre mantenha)
...
ctrl <- list(
  ## defina qual período de tempo que quer utilizar o limiar (ex.: 2011)
  range = which(counts_sts@epoch > weekrange),
  mu0 = list(S = 1,    ## número de termos fourier (harmonics) para incluir
...
Nós precisamos recalcular os termos fourier sem realizar a previsão aqui, uma vez que iremos utilizar 
todos os dados disponíveis para nós (ex.: retrospectivamente). Adicionalmente, nós precisamos calcular 
...
counts <- counts %>% 
  mutate(
    ## corresponde ao t na fórmula
      ## contagem das semanas (provavelmente poderial também utilizar diretamente a variável epiweeks (semanas epidemiológicas))
