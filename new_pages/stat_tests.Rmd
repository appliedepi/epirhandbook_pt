...
O pacote **rstatix** realiza testes estatísticos e gera os resultados de forma que possam ser manipulados ("pipe-friendly"). Os resultados são gerados automaticamente em um quadro de dados, sendo possível realizar operações posteriores com eles. Também é fácil agrupar os dados utilizados nas funções, podendo as estatísticas serem executadas por cada grupo.
...
Por padrão, um resumo estatístico amplo é gerado: n, max, min, mediana, 25%ile, 75%ile, IQR, desvio absoluto mediano (mad), média, desvio padrão, erro padrão, e o intervalo de confiança da média.
...
Você pode especificar um sub-grupo do resumo estatístico a ser gerado, ao fornecer um fos seguintes valores ao argumento `type = `: "full", "common", "robust", "five_number", "mean_sd", "mean_se", "mean_ci", "median_iqr", "median_mad", "quantile", "mean", "median", "min", "max".  
...
Para realizar testes estatísticos de comparação com a função `tbl_summary`, basta adicionar a função `add_p` na tabela e especificar qual teste utilizar. É possível obter os valores de p corrigidos para testes múltiplos ao utilizar a função `add_q`. Utilize o comando `?tbl_summary` para mais detalhes.  
...
linelist %>% 
  ...
  tbl_summary(                                         # produza tabelas resumo
    statistic = age_years ~ "{median} ({p25}, {p75})", # especifique quais estatísticas mostrar (existem valores padrão, então pode deixar os parênteses em branco)
    by = outcome) %>%                                  # especifique a variável de agrupamento
...
Correlações entre variáveis numéricas podem ser investigadas utilizando os pacotes **tidyverse** **corrr**. Assim, é possível realizar os testes de correlação com Pearson, Kendall tau ou Spearman rho. O pacote cria uma tabela e tem uma função para plotar os valores automaticamente.
