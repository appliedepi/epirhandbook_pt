...
A estrutura é **DT[i, j, by]**, separada por 3 partes; os argumentos **i, j** e **by**. O argumento **i** permite filtrar linhas, o argumento **j** permite selecionar colunas e o argumento **by** permite operar em colunas por grupos.
...
* Seleção e cálculo de colunas utilizando o argumento **j**
* Contagem por grupos utilizando o argumento **by**
...
Importamos o conjunto de dados dos casos de uma epidemia simulada de Ébola. Se você quiser baixar os dados para seguir passo a passo, veja as instruções na página [Baixar livro e dados]. O conjunto de dados é importado utilizando a função `import()` do pacote **rio**. Veja a página em [Importar e exportar] para várias formas de importação de dados. Em seguida, utilizamos `data.table()` para converter o quadro de dados em um data table (tabela de dados).

```{r}
linelist <- rio::import(here("data", "linelist_cleaned.xlsx")) %>% data.table()
```

A função `fread()` é utilizada para importar arquivos delimitados por caracteres, como arquivos .csv, diretamente para um formato de data table (tabela de dados). Esta função, e sua contraparte `fwrite()`, utilizada para escrever data.tables como arquivos delimitados, são opções muito rápidas e computacionalmente eficientes para grandes bancos de dados.
...
Relembrando a estrutura **DT[i, j, by]**, podemos filtrar filas usando números de linha ou expressões lógicas. O argumento i é o primeiro; portanto, a sintaxe **DT[i]** ou **DT[i,]** pode ser usada. 

O primeiro exemplo recupera as primeiras 5 linhas do data table (tabela de dados), o segundo exemplo retorna casos com 18 anos ou mais, e o terceiro exemplo gera um subconjunto de casos com 18 anos ou mais, mas não diagnosticados no Hospital Central:
...
O uso de .N no argumento i representa o número total de linhas no data table (tabela de dados). Isto pode ser usado para criar subconjuntos cm base nos números das linhas:  
...
O primeiro exemplo recupera a primeira, terceira e quinta colunas do data table (tabela de dados), o segundo exemplo seleciona todas as colunas, exceto as colunas gender, age, wt_kg e ht_cm. O terceiro exemplo utiliza o envelope `.()` para selecionar as colunas **case_id** e **outcome**.
...
Combinando os argumentos **i** e **j** é possível filtrar linhas e calcular colunas. Usar **.N** no argumento **j** também representa o número total de linhas no data table (tabela de dados) e pode ser útil para retornar o número de linhas após a filtragem.

Nos exemplos a seguir, nós:
* contamos o número de casos que permaneceram mais de 7 dias no hospital
* calculamos a idade média dos casos que morreram no hospital militar
* calculamos o desvio padrão, mediana, idade média dos casos que se recuperaram no hospital central
...
Lembre-se que usar o envelope .() no argumento j facilita o cálculo, retorna uma tabela de dados e permite a nomeação de colunas.
...
* calculamos a altura média e o peso dos casos com 18 anos ou mais, de acordo com o sexo e desfecho (se eles se recuperaram ou morreram)
...
Data.table também permite expressões de encadeamento:
...
Nestes exemplos, estamos seguindo a suposição de que uma linha no data table (tabela de dados) é igual a um novo caso, e assim podemos usar o **.N** para representar o número de linhas no data table (tabela de dados). Outra função útil para representar o número de casos únicos é `uniqueN()`, que retorna o número de valores únicos em uma determinada entrada. Como ilustrado aqui:

````{r}

linelist[, .(uniqueN(gender))] # lembre que o envelope .() no argumento j retorna um data table (tabela de dados)
...
O operador `:=` é utilizado para adicionar ou atualizar dados em um data table (tabela de dados). A adição de colunas pode ser feita das seguintes maneiras:
...
Outras agregações complexas estão além do escopo deste capítulo introdutório, mas a ideia foi fornecer uma alternativa popular e viável ao **dplyr** para agrupamento e limpeza de dados. O pacote **data.table** é um grande pacote que permite um código limpo e legível.
