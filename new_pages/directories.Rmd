...
O **fs**  é um pacote do **tidyverse** quee facilita as interações de diretório, melhorando algumas das funções do R **base**. Nas seções a seguir, usaremos frequentemente as funções do **fs**.  
...
Forneça o caminho da pasta do arquivo para `path =` e decida se deseja mostrar apenas um nível (`recurse = FALSE`) ou todos os arquivos em todos os subníveis (` recurse = TRUE`). Abaixo, usamos `here ()` como uma abreviação para o projeto R e especificamos sua subpasta "dados", que contém todos os dados usados para este manual de R. Nós o configuramos para exibir todos os arquivos em "dados" e suas subpastas (por exemplo, "cache", "modelos de epidemia", "população", "shp" e "clima").
...
exists("linelist")
```
Observe que alguns pacotes do R **base** usam nomes de objetos genéricos como "data" nos bastidores, que aparecerão como TRUE a menos que `inherit = FALSE` seja especificado. Este é um dos motivos para não nomear o conjunto de dados como "data" (de dados).
...
Se você está escrevendo uma função, deve usar `missing ()` do R **base** R para verificar se um argumento está presente ou não, ao invés de `exists ()`.
...
Uma alternativa é `dir.create ()` do R **base** R, que mostrará um erro se o diretório já existir. Em contraste, `dir_create ()` neste cenário será silencioso.
...
`render ()` é uma variação de `source ()` mais frequentemente usada para scripts R markdown. Você fornece o `input =` que é o arquivo markdown R, e também o `output_format =` (normalmente "html_document", "pdf_document", "word_document", "")
