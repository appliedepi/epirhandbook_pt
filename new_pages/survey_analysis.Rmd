...
de [tabelas descritivas](https://epirhandbook.com/descriptive-tables.html) de idade e sexo por estratos de pesquisa. Demonstraremos utilizando o pacote **apyramid**,
pois ele permite proporções ponderadas utilizando nosso objeto de pesquisa criado acima. Outras opções para criar [pirâmides demográficas](https://epirhandbook.com/demographic-pyramids-and-likert-scales.html) são amplamente cobertas nesse capítulo do manual. Também utilizaremos uma função *wrapper* da
**sitrep** chamada `plot_age_pyramid()` que economiza algumas linhas de código para produzir um gráfico comproporções.
...
substancialmente diferente da população original e se a ponderação corrige esta diferença. Para isso, usaremos o pacote **patchwork** para mostrar nossas visualizações **ggplot** lado a lado; para detalhes, veja a seção sobre combinação de lotes no capítulo [dicas ggplot](https://epirhandbook.com/ggplot-tips.html?q=patch#combine-plots) do manual. Visualizaremos nossa população original, nossa população não ponderada de pesquisa e nossa população ponderada de pesquisa. Você
...
conhecidos por vários nomes diferentes, incluindo aluvial, sankey e conjuntos paralelos - os detalhes estão no capítulo sobre [diagramas e gráficos (https://epirhandbook.com/diagrams-and-charts.html#alluvialsankey-diagrams).
...
para repetir sobre várias variáveis e criar uma tabela. Veja o capítulo [iteration](https://epirhandbook.com/iteration-loops-and-lists.html) para mais detalhes do
pacote **purrr**.
...
Também aqui poderíamos escrever uma função para então reiterar sobre múltiplas variáveis usando o pacote **purrr**. Veja o capítulo do manual [iteração](https://epirhandbook.com/iteration-loops-and%20lists.html) para detalhes sobre **purrr**.
