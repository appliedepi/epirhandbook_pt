...
**RStudio** - RStudio é um programa que oferece uma interface gráfica com o usuário (GUI, do inlgês *Graphical User Interface*) para facilitar a utilização do **R**. Mais detalhes [na seção do RStudio](#rstudio).

**Objetos** - Tudo o que você armazena no R - de conjuntos de dados variáveis, listas com nomes de vilarejos e quantidade total de uma população, até o resultado das análises, como os gráficos - são *objetos* que *recebem nomes* e *podem ser utilizados* em comandos posteriores. Mais detalhes [na seção Objetos.](#objects)

**Funções** - Uma função é um código executável que recebe e processa dados, gerando o resultado desse processamento. Mais detalhes [na seção de Funções](#functions).

**Pacotes** - Um pacote R é um conjunto compartilhável e organizado de funções. Mais detalhes [na seção Pacotes](#packages).

**Scripts** - Um script é um documento que contém os códigos e comandos em R. Mais detalhes [na seção Scripts.](#scripts)
