...
### Deduplicação probabilística {.unnumbered}  

A combinação probabilística também pode ser utilizada para deduplicar uma base de dados. Veja a página de deduplicação para outros métodos de deduplicação.  

Aqui, vamos começar com a base `cases`, mas a partir daqui chamaremos ela de `cases_dup`, pois ela tem 2 linhas adicionais que podem ser duplicatas de linhas anteriores:
...
Execute `fastLink()` como anteriormente, mas compare o dataframe `cases_dup` com ele mesmo. Quando os dois dataframes passados para a função são idênticos, ela assume que você quer fazer a deduplicação. Note que nós não especificamos `stringdist.match = ` ou `numeric.match = ` como havíamos feito anteriormente.  
...
Agora, você pode revisar as duplicatas em potencial com `getMatches()`. Passe o dataframe tanto como `dfA = ` como `dfB = `, e passa a saída da função `fastLink()` como `fl.out = `.  `fl.out` deve ser da classe `fastLink.dedupe`, ou em outras palavras, o resultado de `fastLink()`.  
...
Para retornar o número de linhas que são potenciais duplicatas, você pode contar o número de linhas por valores únicos na coluna `dedupe.ids`, e aí filtrar para manter apenas aquelas com mais de uma linha. Nesse caso, isso deixaria as linhas 2 e 3.  
...
Para investigar a totalidade das linhas que poderiam ser duplicatas, coloque o número da linha nesse comando:  

```{r}
# mostra a linha 2 e todas as possíveis duplicatas dela
